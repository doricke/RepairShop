...
main ()
{
  ...
  while ( names.eof != TRUE )
  {
    ...
    write_seq ( seq, 0, seq.total - 1, library );
  ...
  printf ( "\nEnd of Protein Sequences -> Fasta format program.\n" );
}  /* main */
...
t_text  *erase;		/* Unix remove e-mail message file commands file */
{
  ...
  write_seq ( seq, start, end, segment );
...
/* This function reads in an amino acid sequence. */
