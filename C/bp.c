...
#define  EPSILON          1.0e-5   /* convergence tolerance on sum squared error */

#define  INPUT_LAYER      0     /* input layer of the neural network */

#define  MAX_INPUTS       100   /* maximum number of input nodes */

#define  MAX_ITERATIONS   200   /* maximum iterations for conjugate gradient */

#define  MAX_LAYER_NODES  20    /* maximum number of nodes per layer */

#define  MAX_LAYERS       4     /* maximum number of neural network layers */
...
#define  MAX_WEIGHTS      (MAX_LAYERS - 1) * MAX_LAYER_NODES * MAX_LAYER_NODES

#define  MIN_SUM_SQUARED_ERROR  0.001    /* minimum sum squared error */
...
typedef struct {
  ...
  float  gradient [ MAX_LAYER_NODES ];  /* current deltas */
...
t_network  g_network;  /* neural network */

t_patterns  g_patterns;  /* training patterns */
...
main ( )
{
  ...
  read_network ( &g_network, &g_patterns );

  user_randomize ();  /* user initialization of random number generator */

  /* Do a conjugate gradient descent on the sum squared error surface. */
  conjugate_gradient ( &g_network );  

  print_network ( &g_network );  /* print out the final network configuration */
  ...
  for ( layer = 0; layer < MAX_LAYERS; layer++ )
    ...
    for ( node = 0; node < MAX_LAYER_NODES; node++ )
    {
      ...
      for ( lower_node = 0; lower_node < MAX_LAYER_NODES; lower_node++ )
      {
        ...
        (*network).layer [ layer ].node [ node ].gradient [ lower_node ] 
            = 0.0;
      }  /* for (lower_node) */
    ...
    for ( node = 0; node <= (*network).nodes_per_layer [ layer ]; node++ )
    {
      ...
      if (( layer > 0 ) && ( node > 0 ))
      {
        ...
        for ( lower_node = 0; lower_node <= 
          ...
          printf ( "(%7.4f  %7.4f)  ", 
            (*network).layer [ layer ].node [ node ].weight [ lower_node ],
            (*network).layer [ layer ].node [ node ].gradient 
                [ lower_node ] );
...
float  back_propagate ( network, patterns )
t_network   *network;   /* neural network */
t_patterns  *patterns;  /* training patterns */
{
  float  delta;  /* computed delta for the current node */

  float  gradient;  /* computed gradient for an interconnection */

  float  error;    /* Expected output - observed output */
  ...
  float  sum_squared_error = 0.0;    /* sum squared error */
  ...
  for ( node = 1; node <= (*network).nodes_per_layer [ layer ]; node++ )
  {
    ...
    error = (*patterns).io [ (*patterns).current ].output [ node ] -
        (*network).layer [ layer ].node [ node ].output;
    ...
    for ( lower_node = 0; lower_node <= (*network).nodes_per_layer 
        [ layer - 1 ]; lower_node++ )
    {
      gradient = delta * (*network).layer [ layer - 1 ].node [ lower_node ]
          .output;
...
/******************************************************************************/
/* This function applies the training patterns to the neural network. */
float  apply_patterns ( network, patterns )
t_network   *network;   /* neural network */
t_patterns  *patterns;  /* input patterns and expected outputs for training */
{
  float  sum_squared_error = 0.0;    /* sum squared error */
  int    layer;                      /* current layer index */
  int    lower_node;                 /* node index of lower layer */
  int    node;                       /* current node index of current layer */

  /* Initialize the delta weights. */
  for ( layer = INPUT_LAYER + 1; layer < (*network).layers; layer++ )
    for ( node = 1; node <= (*network).nodes_per_layer [ layer ]; node++ )
      for ( lower_node = 0; lower_node <= 
          (*network).nodes_per_layer [ layer - 1 ]; lower_node++ )
        (*network).layer [ layer ].node [ node ].gradient [ lower_node ] 
  ...
  /* Apply each of the training patterns. */
      ...
      (*patterns).current++ )
  {
    ...
    /* Apply a training pattern to the neural network. */
    feed_forward ( network );

    /* Train the neural network. */
    sum_squared_error += back_propagate ( network, patterns );

/* printf ( "AP %d: sum_squared_error = %f\n", (*patterns).current, 
    sum_squared_error ); */
  }  /* for */
...
/******************************************************************************/
/* This function returns the gradient from the last call to nn. */
float  nn ( weights, gradient )
float  weights [];    /* neural network weight vector */
float  gradient [];    /* gradient of the weight vector */
{
  int    layer;                /* current layer index */
  int    lower_node;           /* node index of lower layer */
  int    node;                 /* current node index of current layer */
  float  sum_squared_error;    /* sum squared error of training patterns */
  int    weight = 0;           /* weight vector index */

  /* Copy the weight vector. */
  for ( layer = INPUT_LAYER + 1; layer < g_network.layers; layer++ )
    for ( node = 1; node <= g_network.nodes_per_layer [ layer ]; node++ )
      for ( lower_node = 0; lower_node <= 
          g_network.nodes_per_layer [ layer - 1 ]; lower_node++ )
      {
        weight++;
        g_network.layer [ layer ].node [ node ].weight [ lower_node ]
            = weights [ weight ];
      }  /* for (lower_node) */

  /* Apply the training patterns to the new weights. */
  sum_squared_error = apply_patterns ( &g_network, &g_patterns );
...
float  gradient [];    /* gradient of the weight vector */
{
  ...
  for ( layer = INPUT_LAYER + 1; layer < g_network.layers; layer++ )
    for ( node = 1; node <= g_network.nodes_per_layer [ layer ]; node++ )
      for ( lower_node = 0; 
          lower_node <= g_network.nodes_per_layer [ layer - 1 ]; lower_node++ )
      {
        weight++;
        gradient [ weight ] = - g_network.layer [ layer ].node [ node ]
            .gradient [ lower_node ];
      }  /* for (lower_node) */
}  /* dnn */
...
/******************************************************************************/
/* This function calls the Numerical Recipes in C's conjugate function. */
conjugate_gradient ( network )
t_network  *network;    /* neural network */
{
  int    iterations;                 /* the number of iterations performed */
  int    layer;                      /* current layer index */
  int    lower_node;                 /* node index of lower layer */
  int    no_weights;                 /* the number of weights */
  int    node;                       /* current node index of current layer */
  float  sum_square_error = 99.0;    /* the minimum value of the function */
  float  weights [ MAX_WEIGHTS ];    /* the weight vector */
  ...
  do
  {
    ...
    for ( layer = INPUT_LAYER + 1; layer < (*network).layers; layer++ )
      for ( node = 1; node <= (*network).nodes_per_layer [ layer ]; node++ )
        ...
        {
          no_weights++;
          weights [ no_weights ] = (*network).layer [ layer ].node [ node ]
              .weight [ lower_node ];
        }  /* for (lower_node) */

    printf ( "The number of weights is %d\n", no_weights );

    conjugate ( weights, no_weights, EPSILON, &iterations, &sum_square_error );

    printf ( "Conjugate_gradient completed in %d iterations.\n", iterations );
    printf ( "The sum squared error is %f\n", sum_square_error );

    print_outputs ( network, &g_patterns );    /* Print out the output patterns */
  }
  while ( sum_square_error > MIN_SUM_SQUARED_ERROR );

}  /* conjugate_gradient */


/******************************************************************************/
#define  TOL  2.0e-4    /* tolerance */

/* defining declarations */
int  ncom = 0;                   /* Initial weights */
float  pcom  [ MAX_WEIGHTS ];    /* Global weights p [ ] */
float  xicom [ MAX_WEIGHTS ];    /* Global direction xi [ ] */


/* This function computes a line minimumization. */
line_min ( p, xi, n, fret )
float  p [];     /* an n dimensional point p [1..n] */
float  xi [];    /* an n dimensional direction xi [1..n] */
int    n;        /* the number of weights in the network */
float  *fret;    /* the value of func at the returned location p */
{
...
/******************************************************************************/
/* An "artificial" function of one variable for use by linmin. */
float  f1dim ( x )
float  x;    /* scale amount along xi from p */
{
  int  index;                   /* loop/array index */
  float  f;                     /* function return value */
  float  xt [ MAX_WEIGHTS ];    /* temporary weight vector */

  /* Create a new weight vector scaled x along xicom from pcom. */
  for ( index = 1; index <= ncom; index++ )
    xt [ index ] = pcom [ index ] + x * xicom [ index ];

  /* Evaluate the new weight vector. */
  f = nn ( xt );
...
/******************************************************************************/
#define  EPS  1.0e-10      /* Epsilon for converging to zero */

/* This function does a conjugate gradient descent of the error function. */
conjugate ( p, n, ftol, iter, fret )
float  p [];     /* the weight vector */
...
float  *fret;    /* the minimum value of the function */
{
  ...
  float  g [ MAX_WEIGHTS ];
  float  h [ MAX_WEIGHTS ];
  float  xi [ MAX_WEIGHTS ];


  /* Get the sum squared error of the initial weights. */
  fp = nn ( p );

  /* Get the gradient of the initial weights. */
  dnn ( p, xi );
  ...
  {
    *iter = iteration;    /* return the number of iterations */

    /* line minimumization along xi from p */
    line_min ( p, xi, n, fret );
    ...
    fp = nn ( p );    /* get the sum squared error of new weights */

    dnn ( p, xi );    /* get the gradient vector */
    ...
    for ( index = 1; index <= n; index++ )
    {
      ...
      dgg += ( xi [ index ] + g [ index ] ) * xi [ index ];    /* Polak-Ribiere */
    }  /* for (index) */
    ...
    {
      g [ index ] = - xi [ index ];
      xi [ index ] = h [ index ] = g [ index ] + gam * h [ index ];
    }  /* for (index) */
  }  /* for (iteration) */

  printf ( "Too many iterations in function conjugate.\n" );
