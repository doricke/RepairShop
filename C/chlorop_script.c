...
char *argv [];				/* command line arguments */
{
  ...
  d_strcpy ( cmds.name, "chlorop.cmds" );
  cmds.data = fopen ( cmds.name, "w" );

  fprintf ( cmds.data, "mkdir chlorop\n" );
  fprintf ( cmds.data, "echo Making directory chlorop for results\n" );
  ...
  while ( fof.eof != TRUE )
  {
    ...
    fprintf ( cmds.data, "chlorop %s > chlorop/%s.chlorop\n", name, name );
