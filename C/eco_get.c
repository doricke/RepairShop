...
main ()
{
  ...
  t_text  ftp;			/* ftp commands */
  FILE    *fopen ();		/* file open function */
  char    name [ MAX_LINE ];	/* list file name */


  printf ( "This is the eco Search Results Retrieve program.\n\n" );

  /* Prompt for the input file name. */
  prompt_file ( &fof, "What is the name of the list of file names?" );

  strcpy ( cmds.name, "eco.cmds" );
  cmds.data = fopen ( cmds.name, "w" );
  fprintf ( cmds.data, "ftp -n eco < eco.ftp\n" );
  fclose ( cmds.data );

  strcpy ( ftp.name, "eco.ftp" );
  ftp.data = fopen ( ftp.name, "w" );
  fprintf ( ftp.data, "user ricke pw\n" );
  fprintf ( ftp.data, "prompt\n" );
  fprintf ( ftp.data, "ascii\n" );
  fprintf ( ftp.data, "cd /data1/results/16\n" );
  ...
  while ( fof.eof != TRUE )
  {
    ...
    {
      fprintf ( ftp.data, "cd %s/h_fasta\n", name );
      fprintf ( ftp.data, "lcd %s\n", name );
      fprintf ( ftp.data, "!mkdir h\n" );
      fprintf ( ftp.data, "lcd h\n" );
      fprintf ( ftp.data, "mget *\n" );
      fprintf ( ftp.data, "cd ../h_blast\n" );
      fprintf ( ftp.data, "mget *\n" );
      fprintf ( ftp.data, "lcd ../..\n" );
      fprintf ( ftp.data, "cd ../..\n" );
    }  /* if */
