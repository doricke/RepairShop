...
/*    strcat ( name2, ".seq" ); */
    ...
    {
      fprintf ( cmds.data, "fromembl -IN=%s -NOMON -NUC -LIS=%s.list\n", name, name );
      fprintf ( cmds.data, "tofasta -IN=@%s.list -Def -NOMON -OUT=%s.tfa\n", name, name );
      fprintf ( cmds.data, "rm *.embl\n" );
      fprintf ( cmds.data, "rm %s.list\n", name );
    }  /* if */
