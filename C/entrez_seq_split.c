...
main ()
{
  ...
  {
    /* Find the ACCESSION line. */
    while ( ( strcmp ( retrieve.token, "ACCESSION" ) != 0 ) &&
            ( retrieve.eof != TRUE ) )
...
*/
    ...
    while ( ( strcmp ( retrieve.token, "ORIGIN" ) != 0 ) &&
            ( retrieve.eof != TRUE ) )
    { 
      /* Check if going past the end of a sequence. */
      if ( ( strcmp ( retrieve.token, "LOCUS" ) == 0 ) ||
           ( retrieve.line [ 0 ] == '/' ) )
        printf ( "LOCUS line found in search for data '%s'\n", retrieve.line );
