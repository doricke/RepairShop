...
#define  MAX_GRAIL_LEN  200000  /* maximum length of DNA sequence for GRAIL */
  ...
  while ( fof.eof != TRUE )
  {
    ...
    if ( ( fof.eof != TRUE ) && ( genscan.data != NULL ) )
    {
      ...
      for ( start = 0; start < seq.total; start += MAX_GRAIL_LEN - MIN_GRAIL_OVERLAP )
      {
        ...
        if ( seq.total > MAX_GRAIL_LEN )
          make_segment_name ( &seq, version, mzef.name );
        else
          d_strcpy ( mzef.name, seq.name );
...
t_seq	*seq;		/* DNA sequence */
{
  ...
  /* Check for a FASTA sequence file. */
  if ( (*text).line [ 0 ] == '>' )
