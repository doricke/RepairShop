...
main ()
{
  ...
  printf ( "This is the FASTA Sequence files --> Fasta library program.\n\n" );

  /* Prompt for the input file name. */
  prompt_file ( &names, "What is the list of FASTA sequences file name?" );
  ...
  while ( names.eof != TRUE )
  {
    ...
    while ( in.eof != TRUE )
    {
      if ( d_strlen ( in.line ) > 0 )
        fprintf ( library.data, "%s", in.line );

      /* Get the next line of the input FASTA file. */
      get_line ( &in );
    }  /* while */
  ...
  printf ( "\nEnd of FASTA Sequences -> FASTA library program.\n" );
}  /* main */
