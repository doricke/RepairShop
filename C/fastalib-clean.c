...
main ()
{
  t_text  clean;		/* library of DNA sequences */
  t_text  review;		/* file of DNA sequences to review */
  ...
  /* Open the FASTA library "clean" output file. */
  d_strcpy ( clean.name, source.name );
  clean.name [ d_stridx ( clean.name, "\n" ) ] = '\0';
  clean.name [ d_stridx ( clean.name, "." ) ] = '\0';
  d_strcpy ( review.name, clean.name );
  d_strcat ( clean.name, ".Clean" );
  clean.data = fopen ( clean.name, "w" );

  d_strcat ( review.name, ".Review" );
  review.data = fopen ( review.name, "w" );
  ...
  while ( source.eof != TRUE )
  {
    ...
    {
      fprintf ( review.data, "%s", source.line );
      get_line ( &source );
    }  /* while */
    ...
    if ( seq.total > 0 )
    {
      ...
      fprintf ( clean.data, "%s", description );

      /* Write out the DNA sequence. */
      write_DNA_seq ( &seq, 0, seq.total - 1, &clean );
    }  /* if */
    else
    {
      fprintf ( review.data, "%s", description );
      fprintf ( review.data, "%s", seq_block );
    }  /* else */
  }  /* while */
  ...
  printf ( "\nEnd of FASTA library clean program.\n" );
}  /* main */
