...
#define  MIN_SEQ	50	/* minimum sequence length */
...
/* This function replaces X's with N's and removes trailing N's. */
clip_DNA_seq ( seq )
t_seq  *seq;		/* DNA sequence */
{
  int index = 0;
  ...
  if ( (*seq).total <= 0 )  return;

  /* Replace X's with N's. */
  for ( index = 0; index < (*seq).total; index++ )
  {
    if ( (*seq).base [ index ] == 'x' )  (*seq).base [ index ] = 'n';
    if ( (*seq).base [ index ] == 'X' )  (*seq).base [ index ] = 'N';
  }  /* for */
