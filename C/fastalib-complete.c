...
#define  MAX_LINE	8012	/* maximum line length */
...
main ()
{
  t_text  complete;		/* library of DNA sequences */
  ...
  printf ( "This is the FASTA library select complete sequences program.\n\n" );

  /* Prompt for the input file name. */
  prompt_file ( &source, "What is the list of sequences file name?" );

  /* Open the FASTA library "complete" output file. */
  d_strcpy ( complete.name, source.name );
  complete.name [ d_stridx ( complete.name, "\n" ) ] = '\0';
  /* complete.name [ d_stridx ( complete.name, "." ) ] = '\0'; */
  d_strcat ( complete.name, ".Complete" );
  complete.data = fopen ( complete.name, "w" );
  ...
  while ( source.eof != TRUE )
  {
    ...
    if ( source.eof != TRUE )
    {
      get_next_seq ( &source, &seq, seq_block );
    ...
    if ( ( seq.total > 0 ) &&
         ( description [ d_stridx ( description, "[Complete " ) ] == '[' ) )
    {
      /* Write out the description line. */
      fprintf ( complete.data, "%s", description );

      /* Write out the DNA sequence. */
      write_DNA_seq ( &seq, 0, seq.total - 1, &complete );
    }  /* if */
  }  /* while */

  fclose ( complete.data );
  fclose ( source.data );

  printf ( "\nEnd of FASTA library select Complete sequences program.\n" );
}  /* main */
...
get_next_seq ( text, seq, seq_block )
t_text	*text;		/* ASCII text file */
t_seq	*seq;		/* DNA sequence */
