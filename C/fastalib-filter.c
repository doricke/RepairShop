...
#define  MAX_LINE	12012	/* maximum line length */
...
main ()
{
  ...
  while ( source.eof != TRUE )
  {
    ...
    if ( source.eof != TRUE )
    {
      ...
      /* Convert all IUB codes to N bases. */
      filter_DNA_seq ( &seq );
  ...
  printf ( "\nEnd of FASTA library filter program.\n" );
}  /* main */
...
t_seq	*seq;		/* DNA sequence */
{
  char base;				/* current base */
  int  index;				/* sequence index */


  /* Traverse the DNA sequence converting all IUB codes to N bases. */
  for ( index = 0; index < (*seq).total; index++ )
  {
    switch ( (*seq).base [ index ] )
    {
      case 'A':
      case 'C':
      case 'G':
