...
#define  MAX_LINE	20012	/* maximum line length */
...
main ()
{
  t_text  noiubs;		/* library of DNA sequences */
  ...
  printf ( "This is the FASTA library IUB replacement program.\n\n" );

  /* Prompt for the input file name. */
  prompt_file ( &source, "What is the FASTA library file name?" );

  /* Open the FASTA library "noiubs" output file. */
  d_strcpy ( noiubs.name, source.name );
  noiubs.name [ d_stridx ( noiubs.name, "\n" ) ] = '\0';
  noiubs.name [ d_stridx ( noiubs.name, "." ) ] = '\0';
  d_strcat ( noiubs.name, ".NoIUBs" );
  noiubs.data = fopen ( noiubs.name, "w" );
  ...
  while ( source.eof != TRUE )
  {
    ...
    if ( seq.total > 0 )
    {
      ...
      fprintf ( noiubs.data, "%s", description );

      /* Write out the DNA sequence. */
      write_DNA_seq ( &seq, 0, seq.total - 1, &noiubs );
    }  /* if */
  }  /* while */
  ...
  printf ( "\nEnd of FASTA library IUB replacement program.\n" );
}  /* main */
