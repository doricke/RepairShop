...
#define  MIN_SEQ	120	/* minimum sequence length */
...
main ()
{
  ...
  while ( names.eof != TRUE )
  {
    ...
    /* Trim N's from the DNA sequence off both ends. */
    trim_DNA_seq ( &seq );

    /* Check if the sequence is long enough to write out. */
    if ( seq.total >= MIN_SEQ )
    {
...
t_text  *erase;		/* Unix remove e-mail message file commands file */
{
  ...
  d_strcpy ( segment.name, (*seq).name );
  segment.name [ d_stridx ( segment.name, "-" ) ] = '\0'; 
  d_strcat ( segment.name, "-" );
  itoa ( version, version_str );
  if ( version < 100 )  d_strcat ( segment.name, "0" );
  if ( version < 10  )  d_strcat ( segment.name, "0" );
  ...
  if ( (*cmds).data != NULL )
  {
    ...
    fprintf ( (*cmds).data, "blastn repeats %s V=25 B=25 > %sBR\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "blastx swiss %s V=25 B=25 S=55 S2=55 > %sBS\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "blastx pir %s V=25 B=25 S=55 S2=55 > %sBP\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "blastx genpept %s V=25 B=25 S=55 S2=55 > %sBT\n", 
        segment.name, segment.name );
  }  /* if */
...
setup_fasta ( seq, start, end, version, cmds, erase )
t_seq   *seq;		/* DNA sequence */
long    start;		/* start of DNA sequence segment */
long    end;		/* end of DNA sequence segment */
int	version;	/* current version number */
...
t_text  *erase;		/* Unix remove e-mail message file commands file */
{
  ...
  d_strcat ( segment.name, version_str );
  strlower ( segment.name );
  d_strcpy ( base_name, segment.name );
  ...
  {
    d_strcpy ( segment.name, base_name );
    ...
    fprintf ( (*cmds).data, "%s %%SPH ktup 1 > %sFP\n", 
        segment.name, segment.name );
  }  /* for */
...
/******************************************************************************/
/* This function counts the number of N bases in a sequence range. */
int count_Ns ( seq, start, end, first_N, last_N )
t_seq	*seq;		/* DNA sequence */
int	start;		/* start of range to start counting N bases */
int	end;		/* end of range to stop counting N bases */
int	*first_N;	/* position of first N in the range */
int	*last_N;	/* position of the last N in the range */
{
  int  count = 0;	/* number of N's found */
  int  index;		/* index */
  ...
  for ( index = start; index <= end; index++ )
  {
    if ( (*seq).base [ index ] == 'N' )
    {
      ...
      *last_N = index;
      ...
      if ( *first_N == -1 )  *first_N = index;
    }  /* if */
  }  /* for */

  /* return the number of N's found. */
  return ( count );
}  /* count_Ns */


/******************************************************************************/
/* This function finds stretches of 5 N's or longer for trimming. */
find_long_Ns ( seq, first_N, last_N )
t_seq	*seq;		/* DNA sequence */
int	*first_N;	/* position of first N in the range */
int	*last_N;	/* position of the last N in the range */
{
  ...
  *first_N = *last_N = -1;
  ...
  /* Look for 5 N's or more. */
  for ( index = 0; index < (*seq).total - 4; index++ )
  {
    if ( ( (*seq).base [ index ] == 'N' ) &&
         ( (*seq).base [ index + 1 ] == 'N' ) &&
         ( (*seq).base [ index + 2 ] == 'N' ) &&
...
t_seq   *seq;		/* DNA sequence */
{
  int  first_N;		/* first N in the sequence range */
  int  last_N;		/* last N in the sequence range */


  /* Check for short sequence. */
  if ( (*seq).total < MIN_SEQ )  return;

  /* Trim the beginning of the sequence. */
  while ( ( count_Ns ( seq, 0, 40, &first_N, &last_N ) > 3 ) &&
          ( (*seq).total >= MIN_SEQ ) )
  {
    d_strcpy ( (*seq).base, &((*seq).base [ last_N + 1 ]) );

    (*seq).total -= (last_N + 1);
  }  /* while */

  /* Trim the end of the sequence. */
  while ( ( count_Ns ( seq, (*seq).total - 41, (*seq).total - 1, &first_N, &last_N ) > 3 ) &&
          ( (*seq).total >= MIN_SEQ ) )
  {
    (*seq).total = first_N;
  }  /* while */

  /* Check for lots of N's inside the sequence. */
  if ( count_Ns ( seq, 0, (*seq).total -1, &first_N, &last_N ) > 15 ) 
  {
    /* Clip runs of Ns. */
    find_long_Ns ( seq, &first_N, &last_N );
    ...
    /* See which portion is larger. */
    if ( first_N + 1 >= (*seq).total - last_N + 1 )
    {
      (*seq).total = first_N;
    }
    else
