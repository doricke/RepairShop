...
#define  MAX_BASES      350000  /* maximum DNA sequence length */
...
main ()
{
  t_text  blast_cmds;		/* file of blast search commands */
  ...
  t_text  fasta_cmds;		/* file of fasta search commands */
  ...
  /* Open the commands file. */
  strcpy ( blast_cmds.name, "blast.cmds" );
  blast_cmds.data = fopen ( blast_cmds.name, "w" );
  ...
  for ( program_index = 0; program_index < BLAST_PROGRAMS; program_index++ )
  {
    ...
    for ( ; database_index < database_limit; database_index++ )
      ...
      blast_main ( program_index, database_index, &blast_cmds, &erase, &names );
  }  /* for */

  /* Close the blast commands file. */
  fprintf ( erase.data, "rm %s\n", blast_cmds.name );
  fclose ( blast_cmds.data );
  ...
  for ( database_index = 0; database_index <= MAX_DATABASES; database_index++ )
      ...
      fasta_main ( database_index, &fasta_cmds, &erase, &names, library.name );
  ...
  while ( (*names).eof != TRUE )
  {
    ...
    while ( start + MIN_SEQ < seq.total )
    {
      if ( (*names).eof != TRUE ) 
      {
        ...
        fprintf ( (*cmds).data, "%s %s %s V=20 B=20 -asn1 > ../h_blast/%sB%s%s\n", 
  ...
  while ( (*names).eof != TRUE )
  {
    ...
    while ( start + MIN_SEQ < seq.total )
    {
      if ( (*names).eof != TRUE ) 
      {
        ...
        {
          fprintf ( (*fasta_cmds).data, "fasta3 -o -b 20 -d 20 -h -m 0 -n -Q -z %s ",
              segment_name );
          ...
          fprintf ( (*fasta_cmds).data, "fasta3 -o -b 20 -d 20 -h -i -m 0 -n -Q -z %s ",
              segment_name );
...
t_text  *erase;			/* Unix erase files commands file */
{
  ...
  for ( frame = 0; frame <= 2; frame++ )
  {
    ...
    /* T = GenPept database */
    fprintf ( (*cmds).data, "fasta3 -o -b 20 -d 20 -h -m 0 -Q -z -s blosum50 %s ",
        segment.name );
