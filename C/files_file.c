...
main ()
{
  ...
  printf ( "This is the Sequence files --> single file program.\n\n" );

  /* Prompt for the input file name. */
  prompt_file ( &trapped, "What is the list of sequences file name?" );


  /* Open the single sequences file. */
  strcpy ( sequence.name, trapped.name );
  sequence.name [ stridx ( sequence.name, "\n" ) ] = '\0';
  sequence.name [ stridx ( sequence.name, "." ) ] = '\0';
  strcat ( sequence.name, ".seqs" );

  sequence.data = fopen ( sequence.name, "w" );

  if ( sequence.data != NULL )
  {
    printf ( "\nWriting all sequences to file '%s'.\n", sequence.name );
  }
  else
  {
    printf ( "Could not open sequences file '%s'.\n",
        sequence.name );
    /* exit; */
  }  /* else */
  ...
  while ( trapped.eof != TRUE )
  {
    ...
    if ( ( trapped.eof != TRUE ) && ( sequence.data != NULL ) )
    {
      ...
      write_DNA_line ( &seq, 1, seq.total, &sequence );
    }  /* if */
  ...
  printf ( "\nEnd of DNA Sequences -> single file program.\n" );
}  /* main */
...
/* This function writes out a DNA sequence on one line. */
write_DNA_line ( seq, first, last, text )
t_seq	*seq;		/* DNA sequence */
