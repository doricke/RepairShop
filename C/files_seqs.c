...
#define  BLOCK_SIZE	100000	/* output block size */
...
main ()
{
  ...
  printf ( "This is the Sequence files --> Sequences program.\n\n" );
  ...
  while ( names.eof != TRUE )
  {
    ...
    if ( names.eof != TRUE ) 
      ...
      setup_fasta ( &seq, start, seq.total - 1, version,  
          &fasta_cmds, &erase );
