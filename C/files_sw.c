...
main ()
{
  ...
  long    start = 0;	/* start of DNA sequence */
  int     version;      /* DNA segment number */
  char    version_str [ MAX_LINE ];	/* ascii version number */
  FILE    *fopen ();	/* file open function */


  printf ( "This is the Sequence files --> GenQuest program.\n\n" );
...
/* printf ( "DNA length = %d\n", seq.total ); */
    ...
    {
      printf ( "mail search@caliburn.EPM.ORNL.GOV < %s\n", sequence.name );
      ...
      write_DNA_seq ( &seq, start, seq.total, &sequence );
