...
#define  FRACTAL_REPORTING  50    /* frequency to report fractal sum */

#define  FRACTAL_SUFFIX  "FRACTAL"    /* fractal file name suffix */
...
/******************************************************************************/
/* This function adds a coordinate pair to the sequence. */
int  component_type ( components, seq_index )
t_components	*components;	/* sequence components table */
long            seq_index;      /* current sequence base index */
{
  int	comp_index;	/* sequence component table index */
  int	status;		/* function return status */


  /* Check for an empty components table. */
  if ( (*components).total == 0 )  return ( UNSPECIFIED );

  /* search the sequence component table for the coordinate pair. */
  status = find_pair ( seq_index, seq_index, components, &comp_index );

  /* Check for a components table entry found. */
  if ( ( status == S_FOUND ) || ( status == S_BEGIN_COMP ) ||
       ( status == S_END_COMP ) || ( status == S_SUB_COMP ) )
    status = S_NORMAL;

  if ( status == S_ERROR )
  {
    printf ( "*WARN* status %d error returned from find_pair, file '%s'.\n",
        status, (*components).file_name );
    return ( UNSPECIFIED );
  }  /* if */
  if ( status != S_NORMAL )  return ( UNSPECIFIED );

  /* Return the type of the components table entry. */
  return ( (*components).atom [ comp_index ].type );
}  /* component_type */
...
/******************************************************************************/
/* This function writes out the fractal sums over the length of the sequence. */
write_fractal ( sequence, components, selection )
t_sequence    *sequence;      /* entire sequence */
t_components  *components;    /* sequence components table */
int           selection;      /* sequence component(s) selection */
{
  char  base;                         /* current DNA base */
  int   change;                       /* fractal change amount */
  int   comp_type;                    /* component table entry type */
  FILE  *fopen ();                    /* standard file open routine */
  char  fractal_name [ MAX_LINE ];    /* fractal output file name */
  FILE  *fractal;                     /* fractal output file */
  long  fractal_sum = 0;              /* computed fractal sum */
  long  pattern_sum = 0;              /* fractal sum of patterns */
  long  seq_index;                    /* sequence base index */
  long  sum_all = 0;                  /* fractal sum of entire sequence */


  /* Set up the fractal output file name and open it for writing. */
  strcpy ( fractal_name, (*sequence).filename );

  fractal_name [ charidx ( '.', fractal_name ) + 1 ] = '\0';

  concatenate ( fractal_name, FRACTAL_SUFFIX );

  fractal = fopen ( fractal_name, "w" );

  if ( fractal == NULL )
  {
    printf ( "*WARNING* Opening of fractal output file '%s' failed.\n", 
        fractal_name );
    return;
  }  /* if */
  ...
  {
    base = dna_map [ (*sequence).base [ seq_index ].bits ];

    comp_type = comp_types [ component_type ( components, seq_index ) ];

    /* Fractal sum change delta. */
    change = 0;
    if ( ( base == 'A' ) || ( base == 'G' ) || ( base == 'R' ) ) 
      change = 1;
    else
      if ( ( base == 'C' ) || ( base == 'T' ) || ( base == 'Y' ) )
        change = -1;

    if ( ( selection & comp_type ) != 0 ) fractal_sum += change;
