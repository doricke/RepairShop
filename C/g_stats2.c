...
typedef struct {
  ...
  char          subunit [ MAX_PATTERN ];    /* dominat subunit pattern */
...
/******************************************************************************/
/* This function classifies a pattern match by the dominant subunit. */
classify ( sequence, pattern, start, count )
t_sequence	*sequence;	/* entire sequence */
t_pattern	*pattern;	/* current pattern */
long            start;          /* start of unit pattern match */
int             count;          /* number of unit pattern matches */
{
  long  check;        /* start of subunit check */
  long  sub_check;    /* start of following subunits to check */
  int   sub_count;    /* number of occurances of this subunit */
  int   sub_index;    /* subunit index */


  /* Initialize the subunit pattern name. */
  strcopy ( (*pattern).subunit, (*pattern).name );

  /* Check if subunit pattern match. */
  if ( (*pattern).type != P_SUBUNIT )  return;

  /* Count up the occurances of each subunit pattern. */
  for ( check = start; check < start + (*pattern).length * count; 
        check += (*pattern).length )
  {
    /* Compare to each of the following subunits. */
    sub_count = 0;
    for ( sub_check = check + (*pattern).length;
          sub_check < start + (*pattern).length * count; 
          sub_check += (*pattern).length )
    {
      /* Test each subunit comparison. */
      sub_index = 0;
      while ( ( sub_index < (*pattern).length ) && 
          ( (*sequence).base [ check + sub_index ].bits ==
            (*sequence).base [ sub_check + sub_index ].bits ) )
          sub_index++;

      /* Check for a match. */
      if ( sub_index >= (*pattern).length ) sub_count++;

      /* Check for a dominant subunit. */
      if ( sub_count > count / 2 )
      {
        for ( sub_index = 0; sub_index < (*pattern).length; sub_index++ )
          (*pattern).subunit [ sub_index ] =
              dna_map [ (*sequence).base [ check + sub_index ].bits ];

        (*pattern).subunit [ (*pattern).length ] = '\0';
        return;
      }  /* if */
    }  /* for */
  }  /* for */
...
print_match ( sequence, pattern, components, begin, end )
t_sequence	*sequence;	/* entire sequence */
t_pattern	*pattern;	/* current pattern */
t_components	*components;	/* sequence components table */
long		begin;		/* start of pattern match */
long		end;		/* end of pattern match */
{
  ...
  sub_index = begin - BEFORE_BASES;	/* include surrounding bases */
  printed = 0;
  do 
  {
    /* Separate matched sequence from surrounding bases. */
    if ( sub_index == begin )  fprintf ( (*pattern).pat_file, "  " ); 
    if ( sub_index == end + 1 )
    {
    ...
    if ( ( printed == MAX_PER_LINE ) ||
         ( sub_index == end + AFTER_BASES))
    {
      fprintf ( (*pattern).pat_file, "    %s(%4d)  %6d  %s  ", 
          (*pattern).subunit, end - begin + 1, begin + 1, 
          (*sequence).filename );
      get_types ( components, begin, end, &types );
      print_types ( types, (*pattern).pat_file );
      fprintf ( (*pattern).pat_file, "\n" );

      /* Print the leader for the next line. */
      if ( sub_index < end + AFTER_BASES )
      {
        fprintf ( (*pattern).pat_file, "            " );
  ...
  while ( sub_index <= end + AFTER_BASES );
}  /* print_match */
...
t_components	*components;	/* sequence components table */
{
  ...
  long  start;          /* start of pattern match */
  long	sub_index;	/* pattern index */
      ...
      (*sequence).length - (*pattern).length * (*pattern).minimum + 1; index++ )
  {
    ...
    end = index + (*pattern).length * count - 1;

    /* Include partial matches. */
    if ( (*pattern).type == P_SUBUNIT )
    {
      sub_index = 0;
      while ( ( ( (*sequence).base [ index - sub_index - 1 ].bits &
          (*pattern).base [ (*pattern).length - sub_index - 1 ].bits ) != 0 ) &&
          ( sub_index < (*pattern).length ) )  sub_index++;
      start -= sub_index;
    ...
    /* Report any matches. */
    if ( ( ( end - start + 1 ) < (*pattern).minimum * (*pattern).length ) && 
        ( (*pattern).type == P_SUBUNIT ) )
    {
    ...
    if ( count > 0 )
    {
      /* Classify the pattern by dominant subtype. */
      classify ( sequence, pattern, index, count );

      print_match ( sequence, pattern, components, start, end );

      /* Mark the pattern match area. */
/*      add_pair ( start, end, PATTERN, components ); */

      add_length ( start, end, &((*pattern).histo), H_HISTOGRAM );
...
int          max_index;     /* largest histogram index to print */
{
  ...
  if ( line_index == 1 )  printf ( "%4d", index );
  ...
  for ( seq_index = 0; seq_index < (*sequence).length; seq_index++ )
        ...
        rep_index++ )
    {
      ...
      if (( seq_index + match >= rep_index - 1 ) && ( match > MIN_REPEAT ))
      {
        ...
        classify ( sequence, &repeat, seq_index, 1 );
