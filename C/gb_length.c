...
{
  t_text  lengths;		/* sequence lengths */
  ...
  printf ( "This is the GenBank Lengths program.\n\n" );

  if ( argc > 1 )
  {
    strcpy ( gb.name, argv [ 1 ] );
    open_text_file ( &gb );
  }  /* if */
  else
  ...
  strcpy ( lengths.name, gb.name );
  lengths.name [ d_stridx ( lengths.name, "." ) ] = '\0';
  strcat ( lengths.name, ".length" );
  lengths.data = fopen ( lengths.name, "w" );
  ...
  while ( gb.eof != TRUE )
  {
    ...
    if ( d_stridx ( line, "ACCESSION" ) == 0 )
    {
      strcpy ( accession, &(line [ 12 ]) );
      accession [ d_stridx ( accession, " " ) ] = '\0';
    }  /* if */

    /* Check for LOCUS line. */
    if ( d_stridx ( line, "LOCUS" ) == 0 )
    {
      strcpy ( line, gb.line );

      /* Truncate the line at the bp units. */
      line [ d_stridx ( line, "bp" ) ] = '\0';

      /* Skip the "LOCUS" keyword. */
      strcpy ( line, &(line [ d_stridx ( line, " " ) ]) );
