...
#define  MAX_BASES      1000  /* maximum DNA sequence length */

#define  MAX_LINE	1512	/* maximum line length */
...
{
  t_text  species;		/* Species names */
  t_text  gb;			/* GenBank .seq file */
  FILE    *fopen ();		/* file open function */
  int     index;

  char    accession [ MAX_LINE ];	/* current accession number */
  char    line [ MAX_LINE ];		/* current .seq file line */
  char    taxonomy [ MAX_LINE ];	/* species taxonomy names */

  int     organism;			/* in ORGANISM block flag */


  /* Initialization. */
  accession [ 0 ] = taxonomy [ 0 ] = '\0';
  organism = FALSE;

  printf ( "This is the GenBank species program.\n\n" );

  if ( argc > 1 )
  {
    d_strcpy ( gb.name, argv [ 1 ] );
    open_text_file ( &gb );
  }  /* if */
  else
  ...
  d_strcpy ( species.name, gb.name );
  species.name [ d_stridx ( species.name, "." ) ] = '\0';
  d_strcat ( species.name, ".species" );
  species.data = fopen ( species.name, "w" );

  /* Process the names. */
  while ( gb.eof != TRUE )
  {
    d_strcpy ( line, gb.line );
    line [ d_stridx ( line, "\n" ) ] = '\0';

    /* Check for ACCESSION number line. */
    if ( line [ d_stridx ( line, "ACCESSION" ) ] == 'A' )
    {
      d_strcpy ( accession, &(line [ 12 ]) );

      taxonomy [ 0 ] = '\0';
      organism = FALSE;
    }  /* if */

    /* Check for end of ORGANISM line. */
    if ( organism == TRUE )
    {
    ...
    /* Check if in ORGANISM block. */
    if ( organism == TRUE )
    {
      d_strcat ( taxonomy, &(line [ 12 ]) );
      d_strcat ( taxonomy, " " );
    }  /* if */

    /* Check for ORGANISM line. */
    if ( line [ d_stridx ( line, "ORGANISM" ) ] == 'O' )
    {
      organism = TRUE;
      taxonomy [ 0 ] = '\0';
    }  /* if */
...
/*    if ( ( line [ 0 ] == '/') && ( line [ 1 ] == '/' ) && 
         ( accession [ 0 ] != '\0' ) && ( taxonomy [ 0 ] != '\0' ) )
*/
    if ( ( line [ 0 ] == '/') && ( line [ 1 ] == '/' ) ) 
    {
      fprintf ( species.data, "%s\t%s\n", accession, taxonomy );
