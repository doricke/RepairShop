...
#define  MAX_BASES      29000000  /* maximum DNA sequence length */
...
/* This function reads in a GCG sequence. */
read_GCG_seq ( text, seq )
t_text	*text;		/* ASCII text file */
t_seq	*seq;		/* DNA sequence */
{
  int  end_of_file = FALSE;		/* end of file flag */
  ...
  while ( ( (*text).eof != TRUE ) &&
          ( (*text).line [ d_stridx ( (*text).line, ".." ) ] != '.' ) );

  /* Get the first line of the sequence data. */
  if ( (*text).eof != TRUE )

    get_line ( text );	/* get the next text line */

  /* Skip a blank line. */
  if ( d_strlen ( (*text).line ) < 5 )

    get_line ( text );	/* get the next text line */

printf ( "line = %s\n", (*text).line );

  /* Read in the GCG sequence. */
  (*seq).total = 0;
