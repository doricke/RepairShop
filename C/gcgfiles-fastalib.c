...
/* This function finds the GCG ".." sequence seperator. */
find_dots ( text )
t_text  *text;		/* GCG sequence file */
{
  int  index;				/* line index */


  /* Find the double dots. */
  index = d_stridx ( (*text).line, ".." );
  while ( ( (*text).line [ index ] != '.' ) && ( (*text).eof != TRUE ) )
  {
    /* Get the next line from the GCG sequence file. */
    get_line ( text );
...
t_seq	*seq;		/* DNA sequence */
{
  ...
  /* Find the GCG ".." sequence seperator. */
  find_dots ( text );
