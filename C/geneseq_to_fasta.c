...
#define  MAX_GROUPS	15	/* maximum number of evolutionary groups */
...
{
  t_text  gs;			/* Accession numbers and taxonomy */
  FILE    *fopen ();		/* file open function */


  if ( argc > 1 )
  {
    d_strcpy ( gs.name, argv [ 1 ] );
    open_text_file ( &gs );
  }  /* if */
  else
  {
    printf ( "This is the GeneSeq to FASTA library program.\n\n" );
    prompt_file ( &gs, "What is the name of the GeneSeq .DAT file?" );
  }  /* else */

  /* Process the GeneSeq .DAT file. */
  process_entries ( &gs );
...
/* This function extracts the sequence from the GeneSeq .DAT file. */
extract_sequence ( gs )
t_text	*gs;			/* GeneSeq .DAT file */
{
  int   index;
  char  line [ MAX_LINE ];	/* current line */
  ...
  {
    index = 4;
    while ( ( (*gs).line [ index ] != '\0' ) && 
            ( (*gs).line [ index ] != '\n' ) )
    {
      if ( ( ( (*gs).line [ index ] >= 'A' ) && ( (*gs).line [ index ] <= 'Z' ) ) ||
           ( ( (*gs).line [ index ] >= 'a' ) && ( (*gs).line [ index ] <= 'z' ) ) )
        printf ( "%c", (*gs).line [ index ] );
...
/* This function extracts a FASTA library entry from the GeneSeq .DAT entry. */
process_entries ( gs )
t_text	*gs;			/* GeneSeq .DAT file */
{
  char  line [ MAX_LINE ];	/* current line */
  ...
  if ( (*gs).eof == TRUE )  return;

  /* Copy the annotated sequence entry. */
  while ( (*gs).eof != TRUE ) 
  {
    /* Check for the ID line. */
    if ( d_stridx ( (*gs).line, "ID   " ) == 0 )
    {
      d_strcpy ( line, &((*gs).line [ 5 ]) );
      line [ d_stridx ( line, " " ) ] = '\0';
      printf ( ">%s\n", line );
    }  /* if */

    /* Check for the SQ line. */
    if ( d_stridx ( (*gs).line, "SQ   " ) == 0 )
    {
      /* Skip the SQ line. */
      get_line ( gs );
