...
main ()
{
  ...
  char    name3 [ MAX_LINE ];	/* file name suffix */
  ...
  while ( fof.eof != TRUE )
  {
    ...
    d_strcpy ( name2, name );
    name2 [ d_stridx ( name2, "." ) ] = '\0';
    d_strcpy ( name3, name2 );
    d_strcat ( name2, ".chr21" );
    d_strcat ( name3, ".chr22" );

    fprintf ( cmds.data, "grep chr21 %s > %s\n", name, name2 );
    fprintf ( cmds.data, "grep chr22 %s > %s\n", name, name3 );
