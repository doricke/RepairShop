...
#define MAX_LINE 	162	/* maximum line length */
...
#define TEMP_NAME	"TeMp."	/* Tempoary file name prefix */
...
/* Warning: reverse oridentation of SCAN program. */
stridx ( t, s )
char s[], t[];
{
  int  i, j, k;
...
char	suffix [];		/* pattern output suffix */
{
  ...
  while ( status != S_EOF )
  {
    ...
    if ( ( status == S_NORMAL ) && 
         ...
         ( line [ 0 ] != '!' ) )
    {
      /* Fetch the GCG sequence file. */
      get_seq ( line, local_name );

      /* Open the GCG sequence file. */
      status = open_file ( local_name, &data_file ); 
      fetched_count++;
...
*/
      {
        strcpy ( data_line, "  " );
        status = scan_file ( local_name, data_file, &seq_stats );
...
get_seq ( name, local_name )
char  name [];		/* sequence name or accession number to fetch */
char  local_name [];	/* fetched sequence name */
{
  char  accession [ MAX_LINE ];		/* accession number */
  ...
  if ( name [ stridx ( ":", name ) ] != ':' )
  {
    printf ( "get_seq: unknown sequence name '%s'\n", name );
    return;
  }  /* if */
  strcpy ( accession, &(name [ stridx ( ":", name ) + 1 ]) );

  strcpy ( command, "nice fetch " );
  strcat ( command, name );		/* fetch name */
  strcat ( command, " -out=" );		/* fetch name -out= */
  strcpy ( local_name, TEMP_NAME );	/* TeMp. */
  strcat ( local_name, accession );	/* TeMp.accession */
  strcat ( command, local_name );	/* fetch name -out=TeMp.accession */
  strcat ( command, " -Default" );	/* fetch name -out=TeMp.accession -Default */
