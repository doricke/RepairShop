...
main ()
{
  ...
  while ( sts.eof != TRUE )
  {
    ...
    strcpy ( sequence.name, &(sts.line [ 1 ]) );

    if ( sequence.name [ stridx ( sequence.name, "First" ) ] == 'F' )
      sequence.name [ stridx ( sequence.name, "First" ) ] = '\0';

    while ( sequence.name [ 0 ] == ' ' )
      strcpy ( sequence.name, &(sequence.name [ 1 ]) );

    strcpy ( sequence.name, &(sequence.name [ stridx ( sequence.name, "|" ) + 1 ]) );

    sequence.name [ stridx ( sequence.name, "|" ) ] = '\0';

printf ( "Processing %s\n", sequence.name );

    if ( sequence.name [ stridx ( sequence.name, " " ) ] == ' ' )
      sequence.name [ stridx ( sequence.name, " " ) ] = '\0';

    if ( sequence.name [ stridx ( sequence.name, ":" ) ] == ':' )
      sequence.name [ stridx ( sequence.name, ":" ) ] = '\0';

    for ( line_index = 0; line_index < strlen ( sequence.name ); line_index++ )
    {
...
*/
    ...
    while ( sequence.name [ strlen ( sequence.name ) - 1 ] == '_' )

      sequence.name [ strlen ( sequence.name ) - 1 ] = '\0';
