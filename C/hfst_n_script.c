...
char *argv [];				/* command line arguments */
{
  ...
  char    machine [ MAX_LINE ];	/* target machine name */
  ...
  if ( argc > 2 )
    d_strcpy ( machine, argv [ 2 ] );
  else
    d_strcpy ( machine, "hermes" );

  d_strcpy ( cmds.name, "cmds" );
  cmds.data = fopen ( "cmds", "w" );

  fprintf ( cmds.data, "mkdir hfst\n" );
  fprintf ( cmds.data, "echo Making directory hfst for results\n" );
  ...
  while ( fof.eof != TRUE )
  {
    ...
    fprintf ( cmds.data, "hfst_n_query -target Pfam -mach %s > hfst/%s.fst\n", machine, name );
