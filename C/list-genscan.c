...
main ()
{
  ...
  printf ( "This is the list to GENSCAN Script program.\n\n" );

  /* Prompt for the input file name. */
  prompt_file ( &fof, "What is the name of the list of FASTA file names?" );

  d_strcpy ( cmds.name, fof.name );
  cmds.name [ d_stridx ( cmds.name, "." ) ] = '\0';
  d_strcat ( cmds.name, ".cmds" );
  cmds.data = fopen ( cmds.name, "w" );
  printf ( "\nWriting GENSCAN commands to file %s\n", cmds.name );
  ...
  while ( fof.eof != TRUE )
  {
    ...
    {
      fprintf ( cmds.data, "genscan /data6/progs/genscan/Arabidopsis.smat " );
      fprintf ( cmds.data, "%s -v -cds > %s.genes\n", name, name );
      fprintf ( cmds.data, "java GenScanParser %s.genes\n", name );
    }  /* if */
