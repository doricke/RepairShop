...
static  char  *database [] = {
  "gbest", "genbank", "genpept", "pir", "repeats", "swiss", "gbcu", "fugu", "cs16p13", "c16exons" };
...
t_text  *names;			/* file of sequence names file */
{
  ...
  while ( (*names).eof != TRUE )
  {
    ...
    write_fasta_name ( library, seq.name, start, seq.total, end - start + 1 );
...
t_text  *erase;		/* Unix remove e-mail message file commands file */
{
  ...
  if ( (*cmds).data != NULL )
  {
    ...
    fprintf ( (*cmds).data, "tblastx genbank %s V=20 B=20 > %sBGt\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBGt ../h_blast\n", segment.name );

    fprintf ( (*cmds).data, "blastn gbest %s V=20 B=20 > %sBE\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBE ../h_blast\n", segment.name );
    fprintf ( (*cmds).data, "tblastx gbest %s V=20 B=20 > %sBEt\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBEt ../h_blast\n", segment.name );

    fprintf ( (*cmds).data, "blastn gbcu %s V=20 B=20 > %sBU\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBU ../h_blast\n", segment.name );
    fprintf ( (*cmds).data, "tblastx gbcu %s V=20 B=20 > %sBUt\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBUt ../h_blast\n", segment.name );

    fprintf ( (*cmds).data, "blastn repeats %s V=20 B=20 > %sBR\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBR ../h_blast\n", segment.name );
    fprintf ( (*cmds).data, "tblastx repeats %s V=20 B=20 > %sBRt\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBRt ../h_blast\n", segment.name );
...
*/
    fprintf ( (*cmds).data, "blastn fugu %s V=20 B=20 > %sBF\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sBF ../h_blast\n", segment.name );
  }  /* if */
...
*/
    ...
    /* Fugu comparisons */
    fprintf ( (*cmds).data, "fasta -o -b 5 -d 5 -h -m 0 -n -Q -z " );
    fprintf ( (*cmds).data, "%s %%F ktup 1 > %sFF\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sFF ../h_fasta\n", segment.name );
    fprintf ( (*cmds).data, "fasta -o -b 5 -d 5 -h -i -m 0 -n -Q -z " );
    fprintf ( (*cmds).data, "%s %%F ktup 1 > %sFFr\n", 
        segment.name, segment.name );
    fprintf ( (*cmds).data, "mv %sFFr ../h_fasta\n", segment.name );
  }  /* if */
...
char	library [];	/* name of DNA sequence library for self comparison */
{
  ...
  for ( frame = 0; frame <= 2; frame++ )
  {
    ...
    /* H = GenPept database */
    fprintf ( (*cmds).data, "fasta -o -b 20 -d 20 -h -m 0 -Q -z -s blosum50 " );
