...
typedef struct {
  ...
  char		base [ MAX_BASES ];	/* Each base of sequence */
} t_sequence;
...
t_pattern	*pattern;		/* pattern information */
{
  ...
  capitalize ( token );
  while ( token [ index ] != '\0' )
    (*pattern).base [ (*pattern).length++ ] = token [ index++ ];
...
t_seq_stats	*seq_stats;		/* sequence statistics table */
{
  ...
  while ( status == S_NORMAL )
  {
    ...
    if ( status == S_NORMAL )
    {
      sequence.base [ sequence.length++ ] = seq_char;
...
t_patterns	*patterns;	/* table of search patterns */
{
  ...
  while ( index < (*patterns).total )
  {
    ...
    for ( base = 0; base < (*patterns).pattern [ index ].length; base++ )
      printf ( "%c",
          (*patterns).pattern [ index ].base [ base ] );
    printf ( "\n" );
    index++;
  }  /* while */
  printf ( "\n" );
...
t_components	*components;	/* sequence components table */
{
  ...
  for ( seq_index = 0; seq_index < (*sequence).length; seq_index++ )
        ...
        rep_index++ )
    {
      ...
      match = 0;
      while (( (*sequence).base [ seq_index + match ] ==
          (*sequence).base [ rep_index + match ] ) &&
  ...
  for ( unit = 1; unit <= MAX_UNITS; unit++ )
  {
    ...
    while ( index >= 0 )
    {
      ...
      for ( pat_index = 0; pat_index < unit; pat_index++ )
      {
        ...
        pattern.base [ pat_index ] = pattern.name [ pat_index ];
      }  /* for */
...
int		type;		/* component type */
{
  ...
  for ( seq_index = start; seq_index <= end; seq_index++ )
  {
    ...
    base = (*sequence).base [ seq_index - 1 ];
    ...
    if (( (*sequence).base [ seq_index - 1 ] >= 0 ) &&
        ( (*sequence).base [ seq_index - 1 ] <= 0xF ))
