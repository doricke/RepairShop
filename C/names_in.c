...
main ()
{
  t_text  mail_in;		/* Mail in commands */
  t_text  name_in;		/* Name in file names */
  FILE    *fopen ();		/* file open function */
  int     index;


  printf ( "This is the Name.In program.\n\n" );

  strcpy ( mail_in.name, "Mail.In" );
  mail_in.data = fopen ( mail_in.name, "w" );

  strcpy ( name_in.name, "Name.In" );
  name_in.data = fopen ( name_in.name, "w" );

  /* Generate the names. */
  for ( index = 1; index <= 1000; index++ )
  {
    fprintf ( mail_in.data, "s %1d a%1d\n", index, index );
    fprintf ( name_in.data, "a%1d\n", index );
  }  /* for */

/*  fprintf ( mail_in.data, "quit\n" ); */
