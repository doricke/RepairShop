...
main ()
{
  ...
  printf ( "This is the NCBI retrieved sequences naming program.\n\n" );
  ...
  while ( trapped.eof != TRUE )
  {
    ...
    /* Process the retrieved sequences. */
      ...
      if ( in.eof == TRUE )
        printf ( "No LOCUS line found in file '%s'\n", in.name );
      else

        if ( ( trapped.eof != TRUE ) && ( cmds.data != NULL ) )
