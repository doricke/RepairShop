...
main ()
{
  ...
  fprintf ( cmds.data, "mkdir pfams\n" );
  fprintf ( cmds.data, "echo Making directory pfams for results\n" );
  ...
  while ( fof.eof != TRUE )
  {
    ...
    fprintf ( cmds.data, "hmm_n_query -targ Pfam_fs9 -mach hermes > pfams/%s.pfam\n", name );
