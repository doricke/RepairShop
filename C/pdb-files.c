...
char *argv [];		/* command line arguments */
{
  ...
  while ( sts.eof != TRUE )
  {
    ...
    line_index = d_stridx ( sequence.name, "pdb|" );
    if ( sequence.name [ line_index ] == 'p' )
      d_strcpy ( sequence.name, &(sequence.name [ line_index + 4 ]) );

    line_index = d_stridx ( sequence.name, "sp|" );
    if ( sequence.name [ line_index ] == 's' )
      d_strcpy ( sequence.name, &(sequence.name [ line_index + 3 ]) );

    line_index = d_stridx ( sequence.name, "|" );
