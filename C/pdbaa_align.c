...
#define  CODON_SIZE     3	/* number of bases in DNA codon */
				...
				/* "EGHPRSU" */
#define  DNA_LIBRARIES  7	/* dbEST, GenBank, GenPepet, PIR, Repeats, Swiss, GBCU */
...
#define  MAX_TLC	1	/* maximum number of files to Time Logic Computer */
...
static  char  *database [] = {
  "gbest", "genbank", "genpept", "pir", "repeats", "swiss", "gbcu" };
...
main ()
{
  ...
  printf ( "This is the Swiss Protein Database Align program (Version 0.1).\n\n" );

  /* Open the commands file. */
  erase.data = fopen ( "erase", "w" );

  /* Prompt for the input file name. */
  prompt_file ( &names, "What is the Fasta format database name?" );
  ...
  printf ( "\nEnd of Protein Database Align program.\n" );
}  /* main */
...
/*    if ( (*names).eof != TRUE ) 
      ...
      setup_fasta_protein ( &seq, 0, seq.total - 1, version,  
          &fasta_cmds, erase );
*/
...
t_text  *names;		/* file of sequence names file */
{
  ...
  t_seq   seq;         			/* sequence */
  char    seq_acc [ MAX_LINE ];		/* accession number */
  char    sp_name [ MAX_LINE ];		/* sp|accnum|name */
  t_text  tlc;				/* TLC search files */
  ...
  strcpy ( base_name, "pdbaa" );

  if ( tlc_ftp.data != NULL )
  {
    fprintf ( tlc_ftp.data, "user d_ricke d_ricke\n" );
    fprintf ( tlc_ftp.data, "prompt\n" );
    fprintf ( tlc_ftp.data, "ascii\n" );
    fprintf ( tlc_ftp.data, "cd DeCypher\n" );
    fprintf ( tlc_ftp.data, "mkdir MSA\n" );
    fprintf ( tlc_ftp.data, "cd TLC\n" );
    fprintf ( tlc_ftp.data, "mkdir %s\n", base_name );
    fprintf ( tlc_ftp.data, "cd ../MSA\n" );
    fprintf ( tlc_ftp.data, "mkdir %s\n", base_name );
    fprintf ( tlc_ftp.data, "cd ..\n" );
  }  /* if */
  ...
  while ( (*names).eof != TRUE )
  {
    ...
    read_aa_seq ( names, &seq, sp_name, seq_acc );

    /* Open the output file. */
    strcpy ( fasta.name, seq.name );
    strcat ( fasta.name, ".Fasta" );
    fasta.data = fopen ( fasta.name, "w" );
    write_fasta_name ( fasta, sp_name, 1, seq.total, seq.total );
    ...
    strcpy ( tlc.name, seq_acc );
    strcat ( tlc.name, ".TL" );
    tlc.data = fopen ( tlc.name, "w" );
    write_tlc_header ( &tlc, PROTEIN, base_name );

    fprintf ( tlc_ftp.data, "cd ../MSA/%s\n", base_name );
    fprintf ( tlc_ftp.data, "!echo cd ../MSA/%s\n", base_name );
    fprintf ( tlc_ftp.data, "put %s %s.SEQ\n", fasta.name, seq_acc );
    fprintf ( tlc_ftp.data, "!echo put %s %s.SEQ\n", fasta.name, seq_acc );
    fprintf ( tlc.data, "%s.SEQ\n", seq_acc );
    fprintf ( tlc_ftp.data, "cd ../../search\n" );
    fprintf ( tlc_ftp.data, "!echo cd ../../search\n" );
    fprintf ( tlc_ftp.data, "put %s\n", tlc.name );
    fprintf ( tlc_ftp.data, "!echo put %s\n", tlc.name );
    fprintf ( (*erase).data, "rm %s\n", tlc.name );
    fprintf ( tlc_ftp.data, "rename %s %s9\n", tlc.name,
        tlc.name );
    fprintf ( tlc_ftp.data, "!echo rename %s %s9\n", tlc.name,
        tlc.name );
    finish_tlc_file ( &tlc, PROTEIN, "proteins", base_name, seq.total );
  }  /* while */
  ...
  fprintf ( tlc_cmds.data, "ftp -in server_name < %s\n", tlc_ftp.name );
...
/*  fprintf ( (*tlc_file).data, "[SEARCH ID] %sC\n", (*tlc_file).name ); */
  ...
  {
    fprintf ( (*tlc_file).data, "[MATRIX] D:\\DeCypher\\matrix\\Ricke2.maa\n" );
    fprintf ( (*tlc_file).data, "[OPEN PENALTY] 20\n" );
    fprintf ( (*tlc_file).data, "[EXTEND PENALTY] 4\n" );
    fprintf ( (*tlc_file).data, "[SCALE FACTOR] 3\n" );
  }
  else
  {
    fprintf ( (*tlc_file).data, "[MATRIX] D:\\DeCypher\\matrix\\nt000001.mnt\n" );
    fprintf ( (*tlc_file).data, "[OPEN PENALTY] 1\n" );
    fprintf ( (*tlc_file).data, "[EXTEND PENALTY] 1\n" );
    fprintf ( (*tlc_file).data, "[SCALE FACTOR] 3\n" );
  }  /* else */

  fprintf ( (*tlc_file).data, "[QUERY FORMAT] FASTA/PEARSON\n" );
  fprintf ( (*tlc_file).data, "[QUERY TYPE] AA\n" );

  if ( search_type == PROTEIN )
    fprintf ( (*tlc_file).data, "[QUERY SEARCH] 1\n" );
  else
    fprintf ( (*tlc_file).data, "[QUERY SEARCH] B\n" );

  fprintf ( (*tlc_file).data, "[QUERY PATH] D:\\DeCypher\\MSA\\%s\n", subdirectory );
  fprintf ( (*tlc_file).data, "[QUERY SET] " );
}  /* write_tlc_header */


/* This function writes out the end of a TLC file. */
finish_tlc_file ( tlc_file, search_type, database_name, subdirectory, seq_length )
t_text	*tlc_file;		/* TLC output file */
int	search_type;		/* DNA or PROTEIN */
...
{
  long    threshold;		/* score threshold */
  ...
  fprintf ( (*tlc_file).data, "[TARGET PATH] D:\\DeCypher\\target\n" );
  fprintf ( (*tlc_file).data, "[TARGET SET] %s\n", database_name );
  fprintf ( (*tlc_file).data, "[RESULT PATH] D:\\DeCypher\\TLC\\%s\n", subdirectory );
  fprintf ( (*tlc_file).data, "[MAX SCORES] 1000\n" );
  fprintf ( (*tlc_file).data, "[MAX ALIGNMENTS] 1000\n" );

  /* Set the score threshold based upon sequence length. */
  threshold = seq_length / 3;
  if ( threshold >= 100 )  threshold = 100;
  fprintf ( (*tlc_file).data, "[THRESHOLD] %ld\n", threshold );
...
setup_fasta_protein ( seq, start, end, version, cmds, erase )
t_seq   *seq;		/* DNA sequence */
long    start;		/* start of DNA sequence segment */
long    end;		/* end of DNA sequence segment */
int	version;	/* current version number */
...
read_aa_seq ( text, seq, name, acc_num )
t_text	*text;		/* Fasta format file */
t_seq	*seq;		/* sequence */
char	name [];	/* sequence name */
char	acc_num [];	/* accession number */
{
  int  end_of_file = FALSE;		/* end of file flag */
  int  index;


  strcpy ( name, (*text).line );
  name [ stridx ( name, " " ) ] = '\0';
  name [ stridx ( name, "\n" ) ] = '\0';
...
*/
  ...
  get_line ( text );	/* get first sequence line */
  (*seq).total = 0;
...
t_text  *text;		/* output text file */
{
  ...
  while ( index <= end )
  {
    ...
    map_seq ( seq, index, block, 'F', text, start );
