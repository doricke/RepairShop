...
char *argv [];		/* vector of command line arguments */
{
  ...
  while ( fof.eof != TRUE )
  {
    ...
    if ( ( fof.eof != TRUE ) && ( fof.data != NULL ) )
      ...
      /* Process the Phred .phd file counting the bases over Phred 20. */
      Q20 ( name );

    /* Get the next Phred .phd name. */
    get_line ( &fof );
  }  /* while */
...
/******************************************************************************/
Q20 ( phd_name )
char  phd_name [];		/* Phred .phd file name */
{
  char    base;			/* current base character */
  long    bases;		/* the number of bases in the sequence */
  int     edited;		/* number of edited bases */
  long    first_Q20;		/* coordinate for first Q20 base */
  long    last_Q20;		/* coordinate of last Q20 base */
  t_text  phd_file;		/* Phred .phd text file */
  long    Q20_bases;		/* the number of bases >= Phred 20 */
  long    score;		/* Phred quality score for current base */

  long    get_integer ();	/* specify function type */


  /* Initialize the variables. */
  bases = edited = first_Q20 = last_Q20 = Q20_bases = score = 0;

  /* Open the Phred .phd text file. */
  d_strcpy ( phd_file.name, phd_name );
  open_text_file ( &phd_file );

  /* Find the "BEGIN_DNA" tag line. */
  while ( ( phd_file.eof != TRUE ) &&
          ( phd_file.line [ d_stridx ( phd_file.line, "BEGIN_DNA" ) ] != 'B' ) )

    get_line ( &phd_file );

  /* Count the number of bases & the number greater than Phred 20. */
  while ( ( phd_file.eof != TRUE ) &&
          ( phd_file.line [ d_stridx ( phd_file.line, "END_DNA" ) ] != 'E' ) )
  {
    /* Get the next data line. */
    get_line ( &phd_file );

    /* Check for end of the DNA sequence. */
    if ( ( phd_file.eof != TRUE ) &&
          ( phd_file.line [ d_stridx ( phd_file.line, "END_DNA" ) ] != 'E' ) )
    {
      bases++;

      base = phd_file.line [ 0 ];

      /* Get the Phred quality score for this base. */
      score = get_integer ( &(phd_file.line [ 2 ]) );
      ...
      if ( ( score >= 20 ) && ( score < 90 ) )
      {
        ...
        if ( first_Q20 == 0 )  first_Q20 = bases;
      ...
      /* Count edited bases. */
        ...
        if ( ( score >= 90 ) && ( base != 'n' ) )  edited++;
    }  /* if */

  }  /* while */

  printf ( "%s  %d  %d  %d  %d  %d\n", phd_name, Q20_bases, bases, first_Q20, last_Q20, edited );

  /* Close the Phred .phd text file. */
  fclose ( phd_file.data );
}  /* Q20 */
