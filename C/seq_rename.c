...
main ()
{
  ...
  while ( fof.eof != TRUE )
  {
    ...
    name [ stridx ( name, "*"  ) ] = '\0';
    strcpy ( name2, name );
    name2 [ stridx ( name2, ".seq" ) ] = '\0';
    name2 [ stridx ( name2, ".Seq" ) ] = '\0';

    if ( name2 [ stridx ( name2, "Assem" ) ] == 'A' )
    {
      name2 [ stridx ( name2, "Assem" ) ] = '\0';
      strcat ( name2, "cons" );
    }  /* if */

    strlower ( name2 );

    while ( name2 [ stridx ( name2, "\t" ) ] == '\t' )

      name2 [ stridx ( name2, "\t" ) ] = '_';

    while ( name2 [ stridx ( name2, "?" ) ] == '?' )

      name2 [ stridx ( name2, "?" ) ] = '_';

    while ( name2 [ stridx ( name2, "%2f" ) ] == '%' )
    {
      index = stridx ( name2, "%2f" );
      strcpy ( &(name2 [ index ]), &(name2 [ index + 2 ]) );
      name2 [ index ] = '_';
    }  /* while */
    ...
    while ( name2 [ stridx ( name2, "__" ) ] == '_' )
    {
      index = stridx ( name2, "__" );
      strcpy ( &(name2 [ index ]), &(name2 [ index + 1 ]) );
    }  /* while */

    while ( name2 [ strlen ( name2 ) - 1 ] == '_' )

      name2 [ strlen ( name2 ) - 1 ] = '\0';

    /* Search for strange characters. */
    index = strlen ( name2 ) - 1;
    while ( index >= 0 )
    {
      if ( ( ( name2 [ index ] >= 'A' ) && ( name2 [ index ] <= 'Z' ) ) ||
           ( ( name2 [ index ] >= 'a' ) && ( name2 [ index ] <= 'z' ) ) ||
           ( ( name2 [ index ] >= '0' ) && ( name2 [ index ] <= '9' ) ) ||
             ( name2 [ index ] == '.' ) || ( name2 [ index ] == '_' ) )
        ;
      else
        strcpy ( &(name2 [ index ]), &(name2 [ index + 1 ]) );
      index--;
...
/* This function lower cases a string. */
strlower ( s )
char s [];		/* string */
{
  int  i = 0;
