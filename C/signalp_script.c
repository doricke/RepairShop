...
char *argv [];				/* command line arguments */
{
  ...
  d_strcpy ( cmds.name, "signalp.cmds" );
  cmds.data = fopen ( cmds.name, "w" );

  fprintf ( cmds.data, "mkdir signalp\n" );
  fprintf ( cmds.data, "echo Making directory signalp for results\n" );
  ...
  while ( fof.eof != TRUE )
  {
    ...
    fprintf ( cmds.data, "signalp -t euk %s > signalp/%s.signalp\n", name, name );
