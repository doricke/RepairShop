...
#define  MAX_CMDS	32	/* maximum number of commands files */
...
main ()
{
  t_text  cmds [ MAX_CMDS ];	/* Unix shell commands files */
  t_text  fof;			/* file of file names file */
  FILE    *fopen ();		/* file open function */
  int     index;
  char    name [ MAX_LINE ];	/* list file name */
  char    name2 [ MAX_LINE ];	/* file name suffix */
  int     num_cmds = 16;		/* number of commands files */
  char    version [ MAX_LINE ];	/* version number */
  ...
  if ( fof.name [ d_stridx ( fof.name, "\n" ) ] == '\n' )
    fof.name [ d_stridx ( fof.name, "\n" ) ] = '\0';

  /* Open the commands file. */
  for ( index = 0; index < num_cmds; index++ )
  {
    itoa ( index+1, version );
    d_strcpy ( cmds [ index ].name, fof.name );
    d_strcat ( cmds [ index ].name, version );
    cmds [ index ].data = fopen ( cmds [ index ].name, "w" );
  }  /* for */
  ...
  while ( fof.eof != TRUE )
  {
    fprintf ( cmds [ index % num_cmds ].data, "%s", fof.line );
