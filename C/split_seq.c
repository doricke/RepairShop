...
#define  MAX_BASES      5550000  /* maximum DNA sequence length */
...
main ()
{
  t_text  names;		/* file of sequence file names file */
  ...
  while ( (*names).eof != TRUE )
  {
    ...
    while ( start + MIN_SEQ < seq.total )
    {
      if ( (*names).eof != TRUE ) 
      {
        ...
        make_split_name ( &seq, version, segment_name );

        /* Write out the DNA segment to a local file. */
        write_DNA_segment ( &seq, start, end, segment_name );
...
char    segment_name [];	/* current segment name */
{
  ...
  if ( version < 10000 )  strcat ( segment_name, "0" );
  if ( version < 1000 )  strcat ( segment_name, "0" );
