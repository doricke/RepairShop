...
main ()
{
  ...
  printf ( "This is the STS->GENQUEST program.\n\n" );

  /* Open the commands file. */
  cmds.data = fopen ( "cmds", "w" );

  /* Prompt for the input file name. */
  prompt_file ( &sts, "What is the STS file name?" );

  /* Process the STS sequences. */
  while ( sts.eof != TRUE )
  {
    ...
    strcpy ( sequence.name, sts.token );

    /* Get the first STS sequence line. */
    get_line ( &sts );
    ...
    fprintf ( sequence.data, "TYPE DNA6\n" );
    fprintf ( sequence.data, "TARGET SwissProt\n" ); 
    fprintf ( sequence.data, "METHOD BLAST V=25 B=25\n" );
    fprintf ( sequence.data, "COMMENT %s ", sequence.name );
    fprintf ( sequence.data, "<- %s ", sequence.name );
    fprintf ( sequence.data, "/First= 1\n" );
