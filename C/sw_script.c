...
main ()
{
  ...
  fprintf ( cmds.data, "mkdir sws\n" );
  fprintf ( cmds.data, "echo Making directory sws for results\n" );
  ...
  while ( fof.eof != TRUE )
  {
    ...
    fprintf ( cmds.data, "dc_template_rt -priority 9 -query %s ", name );
    fprintf ( cmds.data, "-template sw_aa -mach hermes > sws/%s.sw-pdbaa\n", name );
