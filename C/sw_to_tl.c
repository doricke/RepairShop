...
#define  SWISS		"swiss"		/* Swiss protein database */
...
t_text  *names;		/* file of sequence names file */
{
  ...
  strcpy ( base_name, "swiss" );

  if ( tlc_ftp.data != NULL )
  {
    fprintf ( tlc_ftp.data, "user anonymous d_ricke@yahoo.com\n" );
    fprintf ( tlc_ftp.data, "prompt\n" );
    fprintf ( tlc_ftp.data, "ascii\n" );
    fprintf ( tlc_ftp.data, "cd SEQ\n" );
  }  /* if */
    ...
    if ( count > 2999 )
{
    ...
    fprintf ( tlc_ftp.data, "rename %s %sC\n", tlc.name,
        tlc.name );
    finish_tlc_file ( &tlc, PROTEIN, "swiss", base_name );

    fprintf ( tlc_ftp.data, "cd ../LANLSEQ\n" );
}  /* if */
  }  /* while */
  ...
  fprintf ( tlc_cmds.data, "ftp -in ftp.timelogic.com < %s\n", tlc_ftp.name );
...
/*  fprintf ( (*tlc_file).data, "[SEARCH ID] %sC\n", (*tlc_file).name ); */
  ...
  {
    fprintf ( (*tlc_file).data, "[MATRIX] c:\\MATRICES\\nt000001.mnt\n" );
  ...
  fprintf ( (*tlc_file).data, "[QUERY PATH] d:\\LANLSEQ\n" );
...
char	subdirectory [];	/* subdirectory name */
{
  ...
  fprintf ( (*tlc_file).data, "[RESULT PATH] d:\\LANLOUT\n" );
  fprintf ( (*tlc_file).data, "[MAX SCORES] 100\n" );
  fprintf ( (*tlc_file).data, "[MAX ALIGNMENTS] 100\n" );
...
char	acc_num [];	/* accession number */
{
  ...
  strcpy ( acc_num, &(name [ 3 ]) );
  strcpy ( (*seq).name, &(acc_num [ stridx ( acc_num, "|" ) + 1 ]) );
  acc_num [ stridx ( acc_num, "|" ) ] = '\0';
