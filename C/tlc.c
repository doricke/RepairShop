...
main ()
{
  ...
  t_text  tlc_cmds;		/* Unix shell TLC commands */
  ...
  while ( names.eof != TRUE )
  {
    ...
    while ( start + MIN_SEQ < seq.total )
    {
      if ( names.eof != TRUE ) 
      {
        ...
        quick_tlc ( &seq, start, end, version,  
            &tlc_cmds, &erase, library.name );
...
/*        setup_blast ( &seq, start, end, version,  
            &cmds, &erase );
*/
        /* Setup up Time Logic Smith-Waterman DNA searches. */
        setup_tlc ( &seq, start, end, version, &tlc_cmds, &erase );
    ...
    while ( start + MIN_SEQ < seq.total )
    {
      if ( names.eof != TRUE ) 
      {
        ...
        quick_tlc_protein ( &seq, start, end, version,  
            &fasta_cmds, &erase, library.name );
...
*/
  ...
  fprintf ( tlc_cmds.data, "echo Hello\n" );
...
setup_tlc ( seq, start, end, version, cmds, erase, library )
t_seq   *seq;		/* DNA sequence */
long    start;		/* start of DNA sequence segment */
long    end;		/* end of DNA sequence segment */
int	version;	/* current version number */
...
char	library [];	/* name of DNA sequence library for self comparison */
{
  ...
  char    vers     [ MAX_LINE ];	/* sequence version */
  ...
  parse_name ( (*seq).name, cosmid, subclone, vers, method );
...
/* This function sets up the Fasta searches. */
quick_tlc ( seq, start, end, version, cmds, erase, library )
t_seq   *seq;		/* DNA sequence */
long    start;		/* start of DNA sequence segment */
long    end;		/* end of DNA sequence segment */
int	version;	/* current version number */
...
/* This function sets up the Fasta searches. */
quick_tlc_protein ( seq, start, end, version, cmds, erase, library )
t_seq   *seq;		/* DNA sequence */
long    start;		/* start of DNA sequence segment */
long    end;		/* end of DNA sequence segment */
int	version;	/* current version number */
...
char	library [];	/* name of DNA sequence library for self comparison */
{
  char    base_name [ MAX_LINE ];	/* Translated sequence filename */
  int     frame;			/* Translation frame */
  char    id [ MAX_LINE ];		/* Translation frame */
  t_text  segment;			/* Fasta formated DNA segment */
  t_seq   rev_seq;			/* reverse & complemented sequence */
  long    rev_end;			/* end of reverse DNA sequence segment */
  long    rev_start;			/* start of reverse DNA sequence segment */
...
parse_name ( name, cosmid, subclone, version, method )
char	name     [];		/* sequence name */
