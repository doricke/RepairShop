...
#define  TRACE		FALSE	/* debug trace flag */
...
static  char  *database [] = {
  ...
  "repeats", "gbcu", "gbest", "genbank", "fugu", "self", "cs16p13", "c16exons" };	/* DNA databases */
...
main ()
{
  ...
  printf ( "This is the TLC files spliting program (Version 0.1).\n\n" );

  /* Prompt for the input file name. */
  prompt_file ( &names, "What is the list of search results file name?" );

  /* Set up TLC search files. */
  tlc_split_main ( &names );

  fclose ( names.data );

  printf ( "\nEnd of TLC files spliting program.\n" );
}  /* main */


/* This function sets up the BLAST search files. */
blast_main ( program_index, database_index, cmds, erase, names )
  ...
  while ( (*names).eof != TRUE )
  {
    ...
    while ( start + MIN_SEQ < seq.total )
    {
      if ( (*names).eof != TRUE ) 
      {
        ...
        fprintf ( (*cmds).data, "%s %s %s V=20 B=20 > ../h_blast/%sB%s\n", 
            blast_program [ program_index ], database [ database_index ], 
            segment_name, segment_name, database_letter [ database_index ] );
...
fasta_main ( database_index, fasta_cmds, erase, names, library_name )
int     database_index;		/* index of current database to search */
    ...
    while ( start + MIN_SEQ < seq.total )
    {
      if ( (*names).eof != TRUE ) 
      {
        ...
        else
        {
          ...
          fprintf ( (*fasta_cmds).data, "ktup 1 > ../h_fasta/%sF%s\n", 
          ...
          fprintf ( (*fasta_cmds).data, "ktup 1 > ../h_fasta/%sF%sr\n", 
              segment_name, database_letter [ database_index ] );
        }  /* else */
...
/******************************************************************************/
/* This function splits a Time Logic Corp. search results file. */
tlc_split_main ( names )
t_text  *names;		/* file of sequence names file */
{
  t_text  in;				/* TLC search results file */
  char    search_id [ MAX_LINE ];	/* query search id */
  char    target_set [ MAX_LINE ];	/* query target set */
  FILE    *fopen ();			/* file open function */


  if ( TRACE == TRUE )
    printf ( "tlc_split_main called\n" );
  ...
  while ( (*names).eof != TRUE )
  {
    ...
    /* Open the search results file for reading. */
    open_text_file ( &in );

    /* Find the [SEARCH id] and [TARGET SET] fields. */
    process_tlc_header ( &in, search_id, target_set );

printf ( "tlc_split_main: search_id '%s' and target_set '%s'\n", search_id, target_set );

  if ( TRACE == TRUE )
    printf ( "tlc_split_main: search_id '%s' and target_set '%s'\n", search_id, target_set );

    /* Process each query sequence search results. */
    while ( in.eof != TRUE )
    {
      if ( in.line [ stridx ( in.line, "[QUERY SEQUENCE]" ) ] == '[' )

        /* Copy the TLC alignments to a new file. */
        copy_alignments ( &in, search_id, target_set );

      /* Get the next search results line. */
      get_line ( &in );
    }  /* while */
  ...
  if ( TRACE == TRUE )
    printf ( "tlc_split_main: finished.\n" );
}  /* tlc_split_main */


/******************************************************************************/
/* This function copies a TLC search results to a seperate file. */
copy_alignments ( in, search_id, target_set )
t_text  *in;			/* TLC search results file */
char    search_id [ ];		/* query search id */
char    target_set [ ];		/* query target set */
{
  char    search_type [ MAX_LINE ];	/* database search identifier */
  char    qtext [ MAX_LINE ];		/* [QTEXT] sequence name */
  char    query_seq [ MAX_LINE ];	/* name of query sequence */
  char    query_sequence [ MAX_LINE ];	/* [QUERY SEQUENCE] line */
  char    seq_name [ MAX_LINE ];	/* query sequence file name */
  t_text  out;				/* new alignment output file */
  FILE    *fopen ();			/* file open function */


  if ( TRACE == TRUE )
    printf ( "copy_alignments called.\n" );

  qtext [ 0 ] = query_sequence [ 0 ] = seq_name [ 0 ] = '\0';

  /* Determine the original sequence name. */
  while ( ( (*in).line [ stridx ( (*in).line, "[QTEXT]" ) ] != '[' ) &&
          ( (*in).line [ stridx ( (*in).line, "[END ALIGNMENTS TIME]" ) ] != '[' ) &&
          ( (*in).eof != TRUE ) )
  {
    if ( (*in).line [ stridx ( (*in).line, "[QUERY SEQUENCE]" ) ] == '[' )

      strcpy ( query_sequence, (*in).line );

    /* Get the next search results header line. */
    get_line ( in );
  }  /* while */

  if ( TRACE == TRUE )
    printf ( "copy_alignments: query_sequence '%s'\n", query_sequence );

  if ( (*in).eof == TRUE )  return;

  if ( (*in).line [ stridx ( (*in).line, "[END ALIGNMENTS TIME]" ) ] == '[' )
  {
    printf ( "*WARNING* [END ALIGNMENTS TIME] reached in copy_alignments.\n" );
    return;
  }  /* if */

  if ( (*in).line [ stridx ( (*in).line, "[QTEXT]" ) ] == '[' )
    strcpy ( qtext, (*in).line );
  else  return;

  if ( TRACE == TRUE )
    printf ( "copy_alignments: qtext '%s'\n", qtext );

  /* Open the output file. */
  strcpy ( query_seq, &(qtext [ 7 ]) );
  query_seq [ stridx ( query_seq, "First" ) ] = '\0';	/* truncate at First */

  /* Trim leading spaces */
  while ( query_seq [ 0 ] == ' ' )
    strcpy ( query_seq, &(query_seq [ 1 ]) );

  /* Truncate at first trailing space. */
  query_seq [ stridx ( query_seq, " " ) ] = '\0';

  if ( blank ( query_seq ) == TRUE )
  {
    strcpy ( query_seq, &(query_sequence [ 16 ]) );

    /* Remove the directory path. */
    while ( query_seq [ stridx ( query_seq, "\\" ) ] == '\\' )

      strcpy ( query_seq, &(query_seq [ stridx ( query_seq, "\\" ) + 1 ]) );

    /* Truncate the .Seq suffix. */
    query_seq [ stridx ( query_seq, ".Seq" ) ] = '\0';
  }  /* if */

  /* Truncate trailing blanks from the query sequence name. */
  while ( query_seq [ strlen ( query_seq ) - 1 ] == ' ' )
    query_seq [ strlen ( query_seq ) - 1 ] = '\0';

  /* Concatenate the search identifier. */
  strcpy ( out.name, "../h/" );
  strcat ( out.name, query_seq );
  strcat ( out.name, &(target_set [ 13 ]) );
  out.name [ stridx ( out.name, " " ) ] = '\0';
  out.name [ stridx ( out.name, "\n" ) ] = '\0';

  if ( TRACE == TRUE )
    printf ( "copy_alignments: output file name '%s'\n", out.name );
...
printf ( "copy_alignments: output file name '%s'\n", out.name );
  ...
  if ( out.data == NULL )
  {
    printf ( "*WARNING* Error opening '%s' as output file name.\n", out.name );
    return;
  }  /* if */

  fprintf ( out.data, "%s", search_id );
  fprintf ( out.data, "%s", target_set );
  fprintf ( out.data, "%s", query_sequence );

  /* Process the alignment. */
  while ( ( (*in).line [ stridx ( (*in).line, "[END ALIGNMENTS TIME]" ) ] != '[' ) &&
          ( (*in).eof != TRUE ) )
  {
    /* Write the current line to the output file. */
    fprintf ( out.data, "%s", (*in).line );
  ...
  if ( TRACE == TRUE )
    printf ( "copy_alignments finished.\n" );
}  /* copy_alignments */


/******************************************************************************/
/* This function searches for important Time Logic header information. */
process_tlc_header ( in, search_id, target_set )
t_text  *in;			/* TLC search results file */
char    search_id [ ];		/* query search id */
char    target_set [ ];		/* query target set */
{
  if ( TRACE == TRUE )
    printf ( "process_tlc_header called.\n" );

  search_id [ 0 ] = target_set [ 0 ] = '\0';    /* initialize */

  /* Process the header. */
  while ( ( (*in).line [ stridx ( (*in).line, "[TARGET SET]" ) ] != '[' ) &&
          ( (*in).line [ stridx ( (*in).line, "[QUERY SEQUENCE]" ) ] != '[' ) &&
          ( (*in).eof != TRUE ) )
  {
    if ( (*in).line [ stridx ( (*in).line, "[SEARCH ID]" ) ] == '[' )

      strcpy ( search_id, (*in).line );
  ...
  if ( TRACE == TRUE )
    printf ( "process_tlc_header: search_id '%s'\n", search_id );

  if ( (*in).line [ stridx ( (*in).line, "[TARGET SET]" ) ] == '[' )

    strcpy ( target_set, (*in).line );

  if ( TRACE == TRUE )
    printf ( "process_tlc_header: target_set '%s'\n", target_set );
}  /* process_tlc_header */
...
char    segment_name [];	/* current segment name */
{
  ...
  if ( version < 100 )  strcat ( segment_name, "0" );
  if ( version <  10 )  strcat ( segment_name, "0" );
  strcat ( segment_name, version_str );
...
setup_fasta_protein ( database_index, seq, start, end, version, cmds, erase )
...
t_text  *erase;			/* Unix erase files commands file */
{
  ...
  for ( frame = 0; frame <= 2; frame++ )
  {
    ...
    fprintf ( (*cmds).data, "fasta -o -b 20 -d 20 -h -m 0 -Q -z -s blosum50 %s ",
        segment.name );
    fprintf ( (*cmds).data, "%%%s ktup 1 > ../h_fasta/%sF%s\n", 
        database_letter [ database_index ], segment.name, database_letter [ database_index ] );
  }  /* for */
