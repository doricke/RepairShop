...
#define  MAX_BASES      50012  /* maximum DNA sequence length */
...
main ()
{
  ...
  t_seq   seq1;			/* DNA sequence #1 */
  t_seq   seq2;			/* DNA sequence #2 */
  ...
  while ( fof.eof != TRUE )
  {
    ...
    /* Copy the sequence off of the line. */
    index = d_stridx ( fof.line, "\t" ) + 1;
    seq1.total = seq2.total = 0;
    while ( fof.line [ index ] != '\t' )
    {
      seq1.base [ seq1.total ] = fof.line [ index ];
      seq1.total++;
      index++;
    }  /* while */
    ...
    while ( ( fof.line [ index ] != '\n' ) && ( fof.line [ index ] != '\0' ) )
    {
      if ( fof.line [ index ] != '\t' )
      {
        seq2.base [ seq2.total ] = fof.line [ index ];
        seq2.total++;
      }  /* if */
      index++;
    }  /* while */
    ...
    if ( ( fof.eof != TRUE ) && ( cmds.data != NULL ) )
    {
      ...
      write_DNA_seq ( &seq1, 0, seq1.total, &cmds );

      fprintf ( cmds.data, ">%s_2\n", name );

      /* Write out the DNA sequence. */
      write_DNA_seq ( &seq2, 0, seq2.total, &cmds );

      fprintf ( cmds.data, "fasta -in1=%s -in2=%s_2 -def\n", name, name );
    }  /* if */
