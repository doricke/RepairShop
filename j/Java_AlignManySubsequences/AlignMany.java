...
public class AlignMany extends Object
{
...
/******************************************************************************/
  ...
  private   byte  word_size = 4;		// comparison word size 


/******************************************************************************/
  // Constructor AlignMany
  public AlignMany ()
  {
    initialize ();
  }  // constructor AlignMany
...
/******************************************************************************/
  // Constructor AlignMany
  public AlignMany ( String [] seqs, byte w_size )
  {
    initialize ();
    setWordSize ( w_size );
    setSequences ( seqs );
...
/******************************************************************************/
  private int computeEndLength ( int [] starts, int [] ends, short [] positions )
  {
    ...
    if ( positions != null )
    {
      for ( int i = 0; i < positions.length; i++ )
        ...
        if ( positions [ i ] > 0 )
        {
          ...
          // Check for a back offset.
          if ( starts [ i ] > ends [ i ] )
          {
            int offset = starts [ i ] - ends [ i ];
            if ( offset > back_offset )  back_offset = offset;
          }  // if
        }  // if
    }  // if
    else
    ...
    // Check for no valid alignments.
    if ( end_length == 0 )  return end_length;

    // Subtract the back offset.
    end_length -= back_offset;
...
/******************************************************************************/
  private void evenLengths ( int [] starts, int [] ends, short [] positions )
  {
    // Determine the current alignment length.
    int max_length = computeEndLength ( starts, ends, positions );
  ...
  private void copyWord ( int [] starts, int [] ends, short [] positions )
  {
    ...
    for ( int i = 0; i < starts.length; i++ )
    {
      ...
      if ( ( starts [ i ] >= 0 ) && ( positions [ i ] > 0 ) )
      {
        ...
        // for ( int j = starts [ i ]; j < ends [ i ] + word_size - 1; j++ )
        for ( int j = starts [ i ]; j < ends [ i ] + word_size; j++ )
...
/******************************************************************************/
  private void printLimits ( int [] starts, int [] ends, short [] positions )
  {
    // Print out header.
    System.out.println ();
...
/******************************************************************************/
      ...
      )
  {
    ...
    int max_len = computeEndLength ( starts, ends, positions );
...
/******************************************************************************/
      ...
      )
  {
    ...
    {
      // Add this subsequence to the alignment.
      starts = matchEnds ( starts, ends, positions, location );
      return;
    }  // if

    // Align the subsequences using a smaller word size.
    SubSequences smaller = new SubSequences ( subsequences, (byte) ( word_size - 1 ), location );

    // Get the subsequence alignments.
    StringBuffer [] segments = smaller.getSubAlignments ();
...
/******************************************************************************/
  public void align ()
  {
    ...
    Alignee [] alignees = new Alignee [ sequences.length ];
    for ( int i = 0; i < sequences.length; i++ )
      alignees [ i ] = new Alignee ( sequences [ i ], word_size );

    // Match the common words between the many sequences.
    CommonWords common_words = new CommonWords ( alignees );

    CommonWord [] sorted_words = common_words.getSortedCommonWords ();
    common_words.initialize ();
    common_words = null;

// System.out.println ( "AlignMany.align: word_size = " + word_size + ", sorted_words = " + sorted_words.length );
...
// System.out.println ( "align: sorted_words[i].count = " + sorted_words [ i ].getCount () );
        ...
        // Copy this word.
        copyWord ( starts, ends, positions );
...
/******************************************************************************/
  public static void main ( String [] args )
  {
    ...
    strs [ 0 ] = "AIGKRMLSTGVEQ";
    strs [ 1 ] = "CVAIGKMLTGVE";
    strs [ 2 ] = "AIGKVMLITGVEQED";

    AlignMany app = new AlignMany ( strs, (byte) 4 );
    app.printAlignment ();
  }  // main
