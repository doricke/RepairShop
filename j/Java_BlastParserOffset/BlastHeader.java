...
/******************************************************************************/
  ...
  private   int  start_offset = 0;			// start of sequence offset
...
/******************************************************************************/
  public void setQueryName ( String value )
  {
    ...
    // Parse the query name for the start offset.
    setStartOffset ();
  }  // method setQueryName
...
/******************************************************************************/
  ...
  {
    // Extract the start offset from the query sequence name.
    int index2 = query_name.lastIndexOf ( '_' );

    // Check for no start offset.
    if ( index2 < 0 )  return;

    int index1 = query_name.substring ( 0, index2 ).lastIndexOf ( '_' );

    // Check for no start offset.
    if ( index1 < 0 )  return;

    int start = InputTools.getInteger ( query_name.substring ( index1 + 1 ) );

    if ( start > 0 )
    {
      start_offset = start - 1;

      query_name = query_name.substring ( 0, index1 );
    }  // if
