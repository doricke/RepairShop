...
{
  String name = sequenceName.toString ();
  ...
  if ( ( line == null ) || ( line.equals ( "null" ) == true ) )
  {
    ...
    else
    {
      ...
      if ( ( line.length () > 1 ) && ( line.charAt ( 0 ) == '>' ) )
      {
        ...
        int index2 = desc.indexOf ( ".scf" );
        if ( ( index2 < index ) && ( index2 > 0 ) )  index = index2;

        // Trim ".bin" from Clemson sequence names.
        index2 = desc.indexOf ( ".bin" );
        if ( ( index2 < index ) && ( index2 > 0 ) )  index = index2;

        // Trim at the first white space character.
        index2 = desc.indexOf ( ' ' );
        ...
        if ( ( index >= 0 ) && ( index < description.length () ) )
          sequenceName.append ( desc.substring ( 0, index ) );
        else
          sequenceName.append ( desc );
