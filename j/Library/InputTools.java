...
protected boolean end_of_file = false;			// End of file flag

protected String fileName = "";				// Input file name

protected BufferedReader input_file;			// input file

protected boolean is_valid_file = true;			// valid input file

protected StringBuffer line = new StringBuffer ( 540 );	// current line of the file

protected long line_number = 0;				// current line number
...
/******************************************************************************/
  public static float getFloat ( String line )
  {
    ...
    float fraction = (float) getInteger ( line.substring ( index + 1 ) );
    while ( fraction >= 1.0f )  fraction /= 10.0f;
...
/******************************************************************************/
  ...
  public StringBuffer nextLine ()
  {
    ...
    catch ( IOException e )
    {
      ...
      System.out.println ( "InputTools.nextLine exception: " + e );
    }  // catch
...
/******************************************************************************/
  public void eliminateDoubleTabs ()
  {
...
/******************************************************************************/
  public static  String eliminateDoubleTabs ( String value )
  {
    int index = value.indexOf ( "\t\t" );

    // Check for no double tabs.
    if ( index < 0 )  return value;

    StringBuffer temp_line = new StringBuffer ( value );

    while ( index >= 0 )
    {
      // Insert a seperator between the two tabs.
      temp_line.insert ( index + 1, "-" );

      index = temp_line.toString ().indexOf ( "\t\t" );
    }  // while
