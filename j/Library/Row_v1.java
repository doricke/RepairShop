...
private static String insertTitle = "Insert";

private static String cancelTitle = "Cancel";

private JFrame queryFrame;		// Frame to query user

private Container frameContainer;	

private int columnCount;		// Number of columns in this table

private String [] columnNames;		// Column names of the table

private String [] columnValues;		// Column values

private JTextField [] columnFields;	// User input fields for each column

private JLabel [] columnLabels;		// Column labels

JButton insertButton = new JButton ( insertTitle );
...
public void releaseResults ()
{
  ...
  {
    System.err.println ( "Row.countRows2 SQLException " + e );
  }  // catch
...
public oracle.sql.CLOB getClobObject ( int field )
{
  ...
  try
  {
    oracle.sql.CLOB my_clob = (oracle.sql.CLOB) resultSet.getObject ( field );
    return my_clob;
    //return (oracle.jdbc2.Clob) resultSet.getObject ( field );
  }  // try
  catch ( Exception e )
  {
    if ( display_messages == true )
...
public String [] getColumn ( int rows )
{
  ...
  try
  {
    while ( resultSet.next () )
    {
      ...
      column [ row++ ] = resultSet.getString ( 1 );
    }  // while 
...
public void setDate ( int column )
{
  ...
  try
  {
    ...
    if ( columnCount > 0 )
    {
      columnValues [ column - 1 ] = date.toString ();
      columnFields [ column - 1 ].setText ( columnValues [ column - 1 ] );
    }  // if 
  }
  catch ( SQLException e )
  {
...
public void setString ( int column, String str )
{
  ...
  try
  {
    ...
    if ( columnCount > 0 )
    {
      columnValues [ column - 1 ] = str;
      columnFields [ column - 1 ].setText ( str );
    }  // if 
  } // try
  catch ( SQLException e )
  {
...
public void setBlob ( int column, java.sql.Blob blob )
{
  ...
  try
  {
    ...
    if ( columnCount > 0 )
    {
      columnValues [ column - 1 ] = blob.toString ();
      columnFields [ column - 1 ].setText ( columnValues [ column - 1 ] );
    }  // if 
  }
  catch ( SQLException e )
...
/*******************************************************************************/
public void newRow ( String table_name )
{
  tableName = table_name;
...
public void newRow ()
{
  if ( tableName == null )
  {
    System.out.println ( "newRow: no tableName set" );
    return;
  }  // if 
  ...
  try
  {
    ...
    ResultSetMetaData tableMeta = resultSet.getMetaData ();

    columnCount = tableMeta.getColumnCount ();

    // Build the column arrays.
    columnNames = new String [ columnCount ];
    columnValues = new String [ columnCount ];
    columnFields = new JTextField [ columnCount ];
    columnLabels = new JLabel [ columnCount ];

    // Build the PreparedStatement from the column names.
    String sql1 = "INSERT INTO " + tableName + " ( ";
    String sql2 = " ) VALUES ( ";
    for ( int col = 1; col <= columnCount; col++ )
    {
      columnNames [ col - 1 ] = tableMeta.getColumnName ( col ); 
      sql1 += columnNames [ col - 1 ];
      sql2 += "?";
      if ( col < columnCount )
      {
        sql1 += ", ";
      ...
      // Set up column specific information.
      columnValues [ col - 1 ] = "";
      columnFields [ col - 1 ] = new JTextField ( "", 20 );
      columnLabels [ col - 1 ] = new JLabel ( columnNames [ col - 1 ] );
      columnLabels [ col - 1 ].setLabelFor ( columnFields [ col - 1 ] );
    }  // for
    sql1 += sql2 + " )";

    // Release the resultSet resources.
    releaseResults ();

    // Release the tableMeta resources.
    tableMeta = null;
  ...
  {
    if ( display_messages == true )
      System.out.println ( "newRow: SQLException " + e );
  }  // catch
...
public void completeRow ()
{
  ...
  frameContainer = queryFrame.getContentPane ();
  frameContainer.setLayout ( new FlowLayout () );

  if ( columnCount <= 0 )  return;

  JPanel panel = new JPanel ();
  panel.setLayout ( new GridLayout ( columnCount, 2 ) );
  
  for ( int col = 0; col < columnCount; col++ )
  {
    panel.add ( columnLabels [ col ] );
    panel.add ( columnFields [ col ] ); 
  }
  ...
  queryFrame.setSize ( 500, 450 );
  queryFrame.setVisible ( true );
  queryFrame.show ();

  queryFrame.addWindowListener ( new WindowHandler () );
    ...
    )
{
  ...
  String sql = "UPDATE " + tableName + " SET " + fieldName1 + 
      " = ? WHERE ( " + fieldName2 + " = ? )";

  // Prepare the update SQL statement.
  prepareInsert ( sql );
  setIntField ( 1, fieldValue1 );
  setIntField ( 2, fieldValue2 );
  executeUpdate ();
}  // method updateField
    ...
    public void actionPerformed ( ActionEvent ae )
    {
      ...
      if ( command.equals ( insertTitle ) )
        ...
        if ( command.equals ( cancelTitle ) )
        {
          ...
          System.out.println ( "Row.ButtonHandler: unknown action: " + ae );
