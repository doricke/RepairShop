...
private BufferedReader sequenceFile;	// sequence file

private boolean iub_bases = false;	// IUB base found flag

private StringBuffer line = new StringBuffer ( 140 );			// Current line of the file
...
/******************************************************************************/
public Sequence ( String sequencePath )
{
...
/******************************************************************************/
public void setRuntime ( Runtime run_time )
{
  runtime = run_time;
}  // method setRuntime
...
private StringBuffer readSeq ()
{
  ...
  while ( end_of_data == false )
  {
    try 
    {
      ...
      line.setLength ( 0 );
      line.append ( sequenceFile.readLine () );
...
/******************************************************************************/
public void writeFastAClob( String clob, OutputTools file )
{
 for( int index = 0; index < clob.length(); index += 50 )
 {
  if( index + 50 >= clob.length() )
   file.println( clob.substring( index ) );
  else
   file.println( clob.substring( index, index + 50 ) );
 } // end: for
} // end: method writeFastAClob


/******************************************************************************/
public void writeFastAClob( String name, String clob, OutputTools file )
{
 if( ( name.length() <= 0 ) || ( clob.length() <= 0 ) ) return;

 file.println( ">" + name + " .." );

 writeFastAClob ( clob, file );
} // end: method writeFastAClob
...
public void openSequenceFile ( )
{
  ...
  try
  {
    sequenceFile = new BufferedReader ( new InputStreamReader 
    ...
    {
      System.out.println ( "Sequence.openSequenceFile: IOException: " + e );
      end_of_file = true;
    }  // catch
  }  // try 
  catch ( IOException e )
  {
    System.out.println ( "Sequence.openSequenceFile: IOException on input file " + e );
  }  /* catch */
...
public void closeSequenceFile ( )
{
  ...
  {
    System.out.println ( "Sequence.closeSequenceFile: IOException while closing input file '" 
        + fileName + "' " + e  );
    return;
  }  // catch
  ...
  if ( ( line == null ) || ( line.equals ( "null" ) == true ) )
  {
    ...
    else
    {
      ...
      if ( line.charAt ( 0 ) == '>' )
      {
        ...
        if ( line.substring ( 0, 5 ).equals ( "LOCUS" ) == true )
        {
...
{
  sequence.setLength ( 0 );	// initialize
